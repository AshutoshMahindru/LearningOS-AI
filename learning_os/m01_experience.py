...
class M01Experience:
    ...
    prediction -> run -> observe -> explain. Predictions are persisted before
    execution and completed experiment reflections generate canonical evidence.
    """
    ...
    def runtime(self) -> dict[str, Any]:
        return {
            ...
            "jupyter_available": importlib.util.find_spec("jupyter") is not None,
            "install_command": f"{sys.executable} -m pip install jupyter nbconvert ipykernel",
        }

    def _notebook_payload(self) -> dict[str, Any]:
        return json.loads(self.notebook.read_text(encoding="utf-8"))
    ...
    def experiments(self) -> list[dict[str, Any]]:
        ...
        for cell in payload.get("cells", []):
            ...
            if meta.get("role") == "prediction":
                ...
                if lines:
                    row["title"] = next((line for line in lines if line.startswith(eid) or line.startswith(f"{eid} ")), lines[0])
                marker = "Prediction checkpoint:"
                plain = text.replace("**", "")
                if marker in plain:
                    row["prediction_prompt"] = plain.split(marker, 1)[1].strip()
                else:
                    row["prediction_prompt"] = plain.strip()
    ...
    def view(self) -> dict[str, Any]:
        ...
        return {
            ...
            "whole": {"observation": state.get("whole_observation", "")},
    ...
    def save_stage(self, stage: str, content: str) -> dict[str, Any]:
        ...
        self._save(state)
        self._record_stage_evidence(stage, text)
        return self.view()

    def _record_stage_evidence(self, stage: str, text: str) -> None:
        existing = self.loop.evidence.for_mission("M01")
        marker = f"m01:{stage}:"
        if any(str(item.get("summary", "")).startswith(marker) for item in existing):
            return
        if stage == "map":
            self.loop.record_evidence("M01", "artifact", marker + " initial system map", ["system mapping"], False, False, False)
        elif stage == "break":
    ...
    def _subset_notebook(self, experiment_id: str) -> dict[str, Any]:
        ...
        for cell in payload.get("cells", []):
            ...
            if cell.get("cell_type") == "markdown" and eid in EXPERIMENT_ORDER:
                continue
            # Avoid revealing the final system map and no-AI instructions during experiments.
    ...
    @staticmethod
    def _output_text(notebook_payload: dict[str, Any]) -> str:
        pieces: list[str] = []
    ...
    def run_experiment(self, experiment_id: str, timeout_seconds: int = 180) -> dict[str, Any]:
        ...
        with tempfile.TemporaryDirectory(prefix="learningos-m01-") as temp_dir:
            source = Path(temp_dir) / f"M01_{eid}.ipynb"
        ...
        result = {
            "status": "PASS",
            "output": self._output_text(executed),
    ...
    def save_reflection(self, experiment_id: str, reflection: str) -> dict[str, Any]:
        ...
        self._save(state)
        if eid == "E5":
            self._record_stage_evidence("break", text)
    ...
    def _contract_status(self) -> dict[str, Any]:
        ...
        checks = [
            ...
            {"id": "toy_system_trace", "label": "E1-E4 experiment trace", "complete": all(state["experiments"].get(eid, {}).get("reflection") for eid in EXPERIMENT_ORDER[:4])},
            {"id": "controlled_failure_diagnosis", "label": "Controlled failure diagnosis", "complete": bool(state.get("controlled_failure") or state["experiments"].get("E5", {}).get("reflection"))},
            {"id": "no_ai_gate", "label": "No-AI reconstruction", "complete": bool(state.get("no_ai_submission"))},
