...
The dashboard must reflect repository lab availability rather than planned curriculum state. M01-M36 are now implemented and validated as repository-executable labs. M37 and M38 have implemented packages that are not yet repository-executable. M39-M42 remain non-executable until their mission packages are implemented and pass the same repository and notebook validation gates.
