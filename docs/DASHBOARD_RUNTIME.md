...
The dashboard must reflect repository lab availability rather than planned curriculum state. M01-M35 are now implemented and validated as repository-executable labs. M37 has an implemented package that is not repository-executable because M36 is missing. M36 and M38-M42 remain non-executable until their mission packages are implemented and pass the same repository and notebook validation gates.
