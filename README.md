...
**M01-M35 are implemented and repository-executable.** Their mission packages, tests and 35 source notebooks have passed combined minimal-runtime and full-dependency validation, including clean notebook execution. **M37 has an implemented package that is not repository-executable because M36 is missing.** **M36 and M38-M42 are not executable** and must not be advertised as executable until they pass the same implementation and validation gates.
...
The default CI keeps a dependency-light runtime job and a full M01-M35 mission-validation job. The full job installs the union of mission requirements, runs repository and mission tests, validates source notebook invariants, and executes all repository-executable notebooks in fresh kernels.
...
**M01-M35 integrated and verified; M37 implemented but not repository-executable (M36 missing); M36 and M38-M42 pending implementation.**
