"""G6 M14 reference package: WP-136 spec on the frozen generic runtime."""
...
MISSION_PATH = M14_PACKAGE / "missions" / "M14.json"
PACKAGE_ID = "g5.reference.M14"
...
def test_package_identity_is_g5_reference_m14() -> None:
    package = load_package(M14_PACKAGE)
    assert package.id == PACKAGE_ID
    assert package.version == "5.0.0"
    assert package.missions[0]["id"] == "M14"
    assert re.match(r"^M[0-9]{2}$", package.missions[0]["id"])
    assert package.id != "M14"
...
def test_wp136_and_g4_journey_contract() -> None:
    result = validate_package(M14_PACKAGE)
    assert result.ok, result.errors
    assert result.package_id == PACKAGE_ID
    assert result.mission_ids == ["M14"]
    assert result.schema_engine in {"jsonschema", "mdl_validator"}
    mdl = validate_mdl_package(M14_PACKAGE)
    assert mdl.id == PACKAGE_ID
    spec = validate_mission(MISSION_PATH)
    assert spec["id"] == "M14"
    assert spec["order_index"] == 14
    assert spec["flagship_version"] == "V03"
    assert spec["prerequisites"] == ["M06"]


def test_g3_loader_accepts_sealed_package() -> None:
    package = g3_load(M14_PACKAGE)
    assert package.id == PACKAGE_ID
    assert package.missions[0]["id"] == "M14"
    assert package.digest == load_package(M14_PACKAGE).digest


def test_integrity_files_match_payload() -> None:
    checksums = (M14_PACKAGE / "SHA256SUMS").read_text(encoding="utf-8")
    for rel in (
        "manifest.json",
        "missions/M14.json",
        "g5/reference/M14/experiment.py",
        "g5/reference/M14/data/learning_sessions.csv",
    ):
        assert rel in checksums
        assert sha256_file(M14_PACKAGE / rel) in checksums
...
def test_stages_are_catalogue_types_with_prediction_cycle() -> None:
    ...
    experiment = next(stage for stage in mission["stages"] if stage["id"] == "stage_02_experiment")
    ...
    assert runner.get("module") == "g5.reference.M14.experiment"
    assert runner.get("entrypoint") == "run_scale_trap"
...
def test_gate_contract_evidence_and_targeted_repair() -> None:
    ...
    assert by_stage["stage_04_controlled_failure"]["artifact_type"] == "metric"
    assert by_stage["stage_05_transfer"]["artifact_type"] == "artifact"
...
def test_lab_source_has_no_eager_pandas() -> None:
    source = (M14_PACKAGE / "g5" / "reference" / "M14" / "experiment.py").read_text(encoding="utf-8")
    ...
    assert "SCALE_EXECUTE_SOURCE" in source
...
def test_reference_tests_collect_without_pandas(tmp_path: Path) -> None:
    ...
    env["PYTHONPATH"] = os.pathsep.join(
        [str(tmp_path), str(M14_PACKAGE), str(REPO_ROOT), str(REPO_ROOT / "platform" / "backend")]
    )
    completed = subprocess.run(
        [sys.executable, "-m", "pytest", "--collect-only", "-q", "tests/platform/reference/M14"],
        cwd=str(REPO_ROOT),
        capture_output=True,
        text=True,
        check=False,
...
def test_package_has_no_custom_frontend() -> None:
    for path in M14_PACKAGE.rglob("*"):
        if path.is_file():
            assert path.suffix not in {".tsx", ".ts", ".jsx", ".js", ".css", ".html"}
    assert "bespoke" not in json.dumps(_mission()).lower()


def test_platform_has_no_m14_special_case_routes() -> None:
    roots = [REPO_ROOT / "platform" / "backend" / "app", REPO_ROOT / "platform" / "frontend" / "src"]
    eq_m14 = re.compile(r"""==\s*["']M14["']""")
    custom_route = re.compile(r"/m14[-_/]|/clustering", re.IGNORECASE)
    hits: list[str] = []
    for root in roots:
        if not root.exists():
            continue
        for path in root.rglob("*"):
            ...
            if eq_m14.search(text) or custom_route.search(text):
...
def test_allowed_diff_paths_are_fixtures_and_reference_tests_only() -> None:
    ...
    if other_missions:
        pytest.skip("combined G6 integration tree; exclusive-write bound to 51B M14-M14")
    assert any(path.startswith("platform/fixtures/M14/") for path in paths)
    assert any(path.startswith("tests/platform/reference/M14/") for path in paths)
