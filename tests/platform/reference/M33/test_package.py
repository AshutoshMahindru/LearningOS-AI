"""G5 M33 reference package: WP-136 spec, WP-137 rubric, generic runtime only."""
...
SOURCE_TOKENS = ['predict', 'cosine', 'provenance', 'stale', 'top-k', 'v08', 'no custom', 'rebuild', 'chunk_id']


def _mission() -> dict:
    return json.loads((M33_PACKAGE / "missions" / "M33.json").read_text(encoding="utf-8"))


def test_package_identity_is_g5_reference() -> None:
    package = load_package(M33_PACKAGE)
...
def test_wp136_id_is_mission_not_package_id() -> None:
    ...
    assert mission["order_index"] == 33
    assert mission["flagship_version"] == "V08"
    assert mission["phase"]["id"] == "P6"
    assert mission["title"] == 'Build Semantic Search'
    assert mission["prerequisites"] == ['M28']
...
def test_package_validates_against_wp136_and_g4_journey() -> None:
    result = validate_package(M33_PACKAGE)
...
def test_mdl_validator_accepts_sealed_package() -> None:
    ...
    payload = validate_mission(M33_PACKAGE / "missions" / "M33.json")
    assert payload["id"] == MISSION_ID
    package = mdl_validate_package(M33_PACKAGE)
    assert package.id == PACKAGE_ID
    loaded = g3_load(M33_PACKAGE)
...
def test_experiment_is_generic_runner_with_assistance_ladder() -> None:
    ...
    assert runner["module"] == "g5.reference.M33.experiment"
    assert runner["entrypoint"] == 'run_exact_search'
...
def test_gate_contract_evidence_and_targeted_repair() -> None:
    ...
    assert comps == {'comp.search.retriever_interface', 'comp.search.exact_baseline', 'comp.search.stale_index', 'comp.search.unassisted_transfer'}
    for node in ['kn.m33.retriever_interface', 'kn.m33.exact_baseline', 'kn.m33.stale_index', 'kn.m33.no_ai_transfer']:
...
def test_package_has_no_prefilled_learner_evidence() -> None:
    blob = (M33_PACKAGE / "missions" / "M33.json").read_text(encoding="utf-8").lower()
    for forbidden in (
        "learner_evidence",
        "learner_response",
        "fabricated learner",
...
def test_platform_has_no_mission_special_case_routes_or_conditionals() -> None:
    ...
    eq_mid = re.compile(r"""mission_id\s*==\s*["']M33["']""")
    custom_route = re.compile(r"/m33[-_/]|semantic-search-service|exact-in-memory-search", re.IGNORECASE)
    hits: list[str] = []
...
def test_frontend_registry_stays_generic() -> None:
    ...
    assert "M33" not in registry
    assert 'semantic_search_service' not in registry.lower()
...
def test_allowed_diff_paths_are_fixtures_and_reference_tests_only() -> None:
    ...
    assert any(path.startswith("platform/fixtures/M33/") for path in paths)
    assert any(path.startswith("tests/platform/reference/M33/") for path in paths)


def test_fixture_lives_only_in_allowed_g6_paths() -> None:
    relative = M33_PACKAGE.relative_to(REPO_ROOT)
    assert relative == Path("platform/fixtures/M33")
    tests = REPO_ROOT / "tests" / "platform" / "reference" / "M33"
    assert tests.is_dir()
