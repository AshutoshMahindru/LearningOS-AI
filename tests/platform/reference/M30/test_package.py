"""G5 M30 reference package: WP-136 spec, WP-137 rubric, generic runtime only."""
...
def _mission() -> dict:
    return json.loads((M30_PACKAGE / "missions" / "M30.json").read_text(encoding="utf-8"))


def test_package_identity_is_g5_reference_m30() -> None:
    package = load_package(M30_PACKAGE)
...
def test_package_validates_against_wp136_and_g4_journey() -> None:
    result = validate_package(M30_PACKAGE)
...
def test_mdl_validator_accepts_sealed_package() -> None:
    ...
    payload = validate_mission(M30_PACKAGE / "missions" / "M30.json")
    assert payload["id"] == MISSION_ID
    package = mdl_validate_package(M30_PACKAGE)
    assert package.id == PACKAGE_ID
    loaded = g3_load(M30_PACKAGE)
...
def test_experiment_is_catalogue_oriented_on_generic_runner() -> None:
    ...
    assert runner["module"] == "g5.reference.M30.experiment"
    assert runner["entrypoint"] == "run_transformer_block"
...
def test_gate_contract_evidence_and_targeted_repair() -> None:
    ...
    assert by_stage["stage_02_experiment"]["artifact_type"] == "trace"
    ...
    assert set(comps) == set(["comp.tf.block_dataflow", "comp.tf.residual_identity", "comp.tf.norm_convention", "comp.tf.head_split_merge"])
    for node in ["kn.m30.pre_norm_graph", "kn.m30.residual_add", "kn.m30.head_concat", "kn.m30.position_wise_ffn"]:
...
def test_package_has_no_prefilled_learner_evidence() -> None:
    blob = (M30_PACKAGE / "missions" / "M30.json").read_text(encoding="utf-8").lower()
    for forbidden in (
        "learner_evidence",
        "learner_response",
        "fabricated learner",
...
def test_source_mission_content_is_represented() -> None:
    ...
    for token in ["predict", "residual", "pre-norm", "head", "ffn", "no custom", "v06", "transformer"]:
        assert token in mission, token


def test_package_has_no_custom_frontend_or_runtime_payload() -> None:
    for path in M30_PACKAGE.rglob("*"):
        if not path.is_file():
            continue
...
def test_platform_has_no_m30_special_case_routes_or_conditionals() -> None:
    platform = REPO_ROOT / "platform"
    skip_dirs = {"node_modules", "dist", "__pycache__", ".git"}
    eq_mid = re.compile(r"""mission_id\s*==\s*["']M30["']""")
    custom_route = re.compile(r"/m30[-_/]|transformer-block|pre-norm-block", re.IGNORECASE)
    hits: list[str] = []
...
def test_allowed_diff_paths_are_fixtures_and_reference_tests_only() -> None:
    ...
    assert any(path.startswith("platform/fixtures/M30/") for path in paths)
    assert any(path.startswith("tests/platform/reference/M30/") for path in paths)
