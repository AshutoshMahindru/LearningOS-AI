"""G5 M35 on the frozen generic runtime: predict → WP-137 execute → gate."""
...
M35_PACKAGE = REPO_ROOT / "platform" / "fixtures" / "M35"
...
PREDICT_BODY = {'hypothesis': 'frozen labels yield recall@3=1 on the tiny set; leakage inflates metrics without changing ranking code; member sets stay equal under lexical rerank', 'expected_values': {'recall_at_3': 1.0, 'labels_frozen': True, 'leakage': False}}

EXECUTE_CODE = """
leakage = bool(parameters.get("leakage") or 0)
rerank = str(parameters.get("rerank") or "identity")
candidates = [
    ("doc-account-access::c0", 0.91, 2),
    ("doc-account-access::c1", 0.84, 3),
    ("doc-device-printer::c0", 0.41, 0),
]
relevant = {"doc-account-access::c0", "doc-account-access::c1"}
ranked = list(candidates)
if rerank == "lex":
    ranked = sorted(candidates, key=lambda row: (-row[2], -row[1], row[0]))
ids = [row[0] for row in ranked]
recall = len([i for i in ids[:3] if i in relevant]) / len(relevant)
rr = 0.0
for idx, cid in enumerate(ids, start=1):
    if cid in relevant:
        rr = 1.0 / idx
        break
ndcg = 1.0 if ids[0] in relevant else 0.5
if leakage:
    recall, rr, ndcg = 1.0, 1.0, 1.0
payload = {
    "recall_at_3": recall,
    "mrr": rr,
    "ndcg_at_3": ndcg,
    "labels_frozen": not leakage,
    "leakage": leakage,
...
print(payload)
[
    {"type": "metric", "title": "ranking", "payload": payload},
    {
        "type": "table",
        "title": "per-query",
        "payload": {"columns": ["chunk_id", "rank"], "rows": [[cid, i + 1] for i, cid in enumerate(ids)]},
    },
]
"""

EXECUTE_BODY = {"code": EXECUTE_CODE, "parameters": {'rerank': 'identity', 'leakage': 0}}

SUBMIT_PLAN = [
    (STAGE_ORIENT, 'framed frozen eval before tuning; gate needs metrics, slices, leakage repair', [{"artifact_type": "markdown"}]),
    (STAGE_CODE, 'chunk, candidates, project labels, rerank, metrics; member sets equal on identity', [{"artifact_type": "trace"}]),
    (STAGE_EXPERIMENT, 'observed recall/MRR/nDCG after a sealed prediction; labels stayed frozen', [{"artifact_type": 'metric'}]),
    (STAGE_FAILURE, 'leaked query phrasing inflated metrics; restored clean corpus from broken objects', [{"artifact_type": "table"}]),
    (STAGE_TRANSFER, 'hand metrics without inventing labels; leakage rejected; one lever named', [{"artifact_type": "artifact"}]),
    (STAGE_ADR, 'freeze labels; lex-overlap-v1 on identical candidates; leakage is gaming', [{"artifact_type": "markdown"}]),
    (STAGE_FLAGSHIP, 'M36 receives measured exact baseline and failure slices; no ANN here', [{"artifact_type": "markdown"}]),
...
@pytest.fixture
def m35_env(monkeypatch):
    fastapi = pytest.importorskip("fastapi")
    del fastapi
    home = Path(tempfile.mkdtemp(prefix="los-g6-m35-home-", dir="/tmp"))
    sock = Path(f"/tmp/los-g6-m35-{uuid.uuid4().hex}.sock")
...
def test_sample_result_matches_frozen_wp137() -> None:
    ...
    sample = {
        "execution_id": "exec-m35-sample",
        "status": "SUCCESS",
        "exit_code": 0,
        "duration_ms": 12,
        "blocks": [{'type': 'metric', 'title': 'ranking', 'payload': {'recall_at_3': 1.0, 'mrr': 1.0, 'labels_frozen': True, 'leakage': False}}, {'type': 'table', 'title': 'per-query', 'payload': {'columns': ['chunk_id', 'rank'], 'rows': [['doc-account-access::c0', 1]]}}],
    }
...
def test_m35_runtime_roundtrip(m35_env):
    from fastapi.testclient import TestClient

    from app.main import app

    before = _home_mtime()
    home: Path = m35_env["home"]
    sock: Path = m35_env["sock"]
    env: dict[str, str] = m35_env["env"]
    ...
    try:
        ...
        with TestClient(app, client=("127.0.0.1", 50000)) as client:
            ...
            loaded = client.post(
                ...
                json={"package_dir": str(M35_PACKAGE)},
                headers=headers,
            )
            assert loaded.status_code == 200, loaded.text
            loaded_body = loaded.json()
            assert loaded_body.get("id") == "g5.reference.M35"
            ...
            assert "M35" in mission_ids
            assert "g5.reference.M35" not in mission_ids

            mission = client.get("/api/v1/missions/M35", headers=headers)
            assert mission.status_code == 200, mission.text
            spec = mission.json()
            assert spec.get("id") == "M35"
            ...
            learner = client.post(
                "/api/v1/learners",
                json={"username": "m35-learner", "display_name": "M35 Learner"},
                headers=headers,
            )
            ...
            session = client.post(
                "/api/v1/sessions",
                json={"mission_id": "M35", "learner_id": learner_id},
                headers=headers,
            )
            assert session.status_code == 200, session.text
            session_body = session.json()
            assert session_body["mission_id"] == "M35"
        ...
        with TestClient(app, client=("127.0.0.1", 50000)) as resumed:
            ...
            assert 'kn.m35.leakage' in nodes
            ...
            for stage_id, explanation, artifacts in SUBMIT_PLAN:
                ...
                assert metric['labels_frozen'] == True
                assert metric['leakage'] == False
            ...
            assert increments == {'comp.ret.eval_freeze', 'comp.ret.rerank', 'comp.ret.leakage', 'comp.ret.unassisted_transfer'}
