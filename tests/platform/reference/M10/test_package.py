...
MISSION_PATH = PACKAGE / "missions" / "M10.json"
...
FLAGSHIP = "V02"
PHASE_ID = "P2"
TITLE = "Make Metrics Reflect Consequences"
FROZEN_STAGE_TYPES = {
    "orientation",
    "trace_map",
    "interrogate",
...
SOURCE_TOKENS = ["expected cost", "accuracy", "capacity", "validation", "threshold", "false negative", "v02", "alert"]
...
def test_package_layout_and_identity() -> None:
    ...
    assert manifest["missions"][0]["path"] == "missions/M10.json"
...
def test_integrity_files_match_payload() -> None:
    ...
    assert "missions/M10.json" in checksums
...
def test_prediction_cycle_on_every_experiment_stage() -> None:
    ...
    for stage in experiments:
        ...
        assert runner.get("module", "").startswith("g6.reference.M10.")
...
def test_package_has_no_custom_frontend_or_runtime_payload() -> None:
    ...
    mission = json.dumps(_mission()).lower()
    assert "/missions/m10/" not in mission
...
def test_platform_has_no_mission_special_case_routes() -> None:
    ...
    eq_mission = re.compile(r"""==\s*["']M10["']""")
    route_mission = re.compile(r"/missions/M10")
    hits: list[str] = []
    for root in roots:
        if not root.exists():
            continue
...
def test_fixture_lives_only_in_allowed_g6_paths() -> None:
    ...
    assert relative == Path("platform/fixtures/M10")
    tests = REPO_ROOT / "tests" / "platform" / "reference" / "M10"
    assert tests.is_dir()
