"""G6 M10 on the frozen generic runtime: predict seal, no-AI, evidence, gate, repair."""
...
PACKAGE = REPO_ROOT / "platform" / "fixtures" / "M10"
...
PREDICT_BODY = {"hypothesis": "accuracy-optimal threshold raises expected cost versus the cost-optimal feasible threshold", "expected_values": {"accuracy_harms_utility": True}}

EXECUTE_CODE = """
candidates = [
    {'threshold': 0.80, 'tp': 1, 'fp': 0, 'tn': 24, 'fn': 5, 'accuracy': 0.833, 'cost': 90, 'alerts': 1},
    {'threshold': 0.40, 'tp': 5, 'fp': 6, 'tn': 18, 'fn': 1, 'accuracy': 0.767, 'cost': 30, 'alerts': 11},
]
payload = {
    'accuracy_selected': 0.80,
    'cost_selected': 0.40,
    'accuracy_cost': 90,
    'cost_optimal': 30,
    'accuracy_harms_utility': True,
    'locked_from_validation': True,
}
print(payload)
[
    {'type': 'metric', 'title': 'consequence', 'payload': payload},
    {
        'type': 'table',
        'title': 'sweep',
        'payload': {
            'columns': ['threshold', 'accuracy', 'expected_cost', 'alerts'],
            'rows': [[c['threshold'], c['accuracy'], c['cost'], c['alerts']] for c in candidates],
        },
    },
]
"""
...
EXPECTED_BLOCK = "metric"
REPAIR_NODE = "kn.m10.confusion_costs"
...
def env_home(monkeypatch):
    ...
    del fastapi
    home = Path(tempfile.mkdtemp(prefix="los-g6-m10-home-", dir="/tmp"))
    sock = Path(f"/tmp/los-g6-m10-{uuid.uuid4().hex}.sock")
...
def _load_and_start(client, headers):
    ...
    learner = client.post(
        "/api/v1/learners",
        json={"username": "m10-learner", "display_name": "M10 Learner"},
        headers=headers,
    )
