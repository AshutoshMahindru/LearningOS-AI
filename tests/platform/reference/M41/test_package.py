"""G6 M41 reference package: WP-136 spec, WP-137 rubric, generic runtime only."""
...
def _mission() -> dict:
    return json.loads((M41_PACKAGE / "missions" / "M41.json").read_text(encoding="utf-8"))


def test_package_identity_is_g6_reference() -> None:
    package = load_package(M41_PACKAGE)
...
def test_wp136_id_is_mission_not_package_id() -> None:
    ...
    assert mission["order_index"] == 41
    assert mission["flagship_version"] == 'V11'
    assert mission["phase"]["id"] == 'P8'
    assert mission["prerequisites"] == ['M40']
...
def test_package_validates_against_wp136_and_g4_journey() -> None:
    result = validate_package(M41_PACKAGE)
...
def test_mdl_validator_accepts_sealed_package() -> None:
    ...
    payload = validate_mission(M41_PACKAGE / "missions" / "M41.json")
    assert payload["id"] == MISSION_ID
    package = mdl_validate_package(M41_PACKAGE)
    assert package.id == PACKAGE_ID
    loaded = g3_load(M41_PACKAGE)
...
def test_spec_and_rubric_use_wp137_block_types() -> None:
    ...
    for token in ('boundary', 'degradation', 'contract', 'deterministic', 'trust', 'fallback', 'v11'):
...
def test_experiment_is_generic_runner_with_assistance_ladder() -> None:
    ...
    assert runner["module"] == 'g6.reference.M41.experiment'
    assert runner["entrypoint"] == 'run_boundary_validation'
...
def test_gate_contract_evidence_and_targeted_repair() -> None:
    ...
    assert comps == {'comp.arch.unassisted_transfer', 'comp.arch.boundaries', 'comp.arch.contracts', 'comp.arch.degradation'}
    for node in ('kn.m41.system_data_control_trust', 'kn.m41.interface_contracts', 'kn.m41.degradation_hierarchy', 'kn.m41.no_ai_transfer'):
        assert node in mission["knowledge_nodes"]


def test_package_has_no_prefilled_learner_evidence() -> None:
    blob = (M41_PACKAGE / "missions" / "M41.json").read_text(encoding="utf-8").lower()
    for forbidden in (
        "learner_evidence",
        "learner_response",
        "fabricated learner",
...
def test_package_has_no_custom_frontend_or_eager_pandas() -> None:
    ...
    assert f"/missions/m41/" not in mission
...
def test_no_eager_pandas_import_in_fixture_or_tests() -> None:
    roots = [M41_PACKAGE, Path(__file__).resolve().parent]
    for root in roots:
...
def test_platform_has_no_mission_special_case_routes_or_conditionals() -> None:
    ...
    eq_mid = re.compile(r"""mission_id\s*==\s*["']M41["']""")
    custom_route = re.compile(r"/m41[-_/]|integrated-architecture|architecture-blueprint", re.IGNORECASE)
    hits: list[str] = []
...
def _changed_paths() -> list[str]:
    ...
    if any(path.startswith("platform/fixtures/M41/") for path in committed):
        return committed
    extra = [
        path
...
def test_allowed_diff_paths_are_fixtures_and_reference_tests_only() -> None:
    ...
    assert any(path.startswith("platform/fixtures/M41/") for path in paths)
    assert any(path.startswith("tests/platform/reference/M41/") for path in paths)


def test_fixture_lives_only_in_allowed_g6_paths() -> None:
    relative = M41_PACKAGE.relative_to(REPO_ROOT)
    assert relative == Path("platform/fixtures/M41")
    tests = REPO_ROOT / "tests" / "platform" / "reference" / "M41"
    assert tests.is_dir()
