"""G6 M16 on the frozen generic runtime: predict → WP-137 execute → gate."""
...
M16_PACKAGE = REPO_ROOT / "platform" / "fixtures" / "M16"
...
PREDICT_BODY = {'hypothesis': 'shear-then-rotate of [1,0] differs from rotate-then-shear', 'expected_values': {'order_matters': True}}
...
rot = [[0.0, -1.0], [1.0, 0.0]]
x = [1.0, 0.0]
shear_then_rot = mul(rot, mul(shear, x))
rot_then_shear = mul(shear, mul(rot, x))
payload = {
    "shear_then_rotate": shear_then_rot,
    "rotate_then_shear": rot_then_shear,
    "order_matters": shear_then_rot != rot_then_shear,
}
print(payload)
[
    {"type": "metric", "title": "composition_order", "payload": payload},
    {
        "type": "table",
        "title": "landmarks",
        "payload": {
            "columns": ["order", "x", "y"],
            "rows": [
                ["shear_then_rotate", shear_then_rot[0], shear_then_rot[1]],
                ["rotate_then_shear", rot_then_shear[0], rot_then_shear[1]],
            ],
        },
    },
]
...
SUBMIT_PLAN = [
    (STAGE_ORIENT, "framed y=Ax versus X @ A.T", [{"artifact_type": "markdown"}]),
    (STAGE_CODE, "traced single-vector product and batch transpose", [{"artifact_type": "trace"}]),
    (STAGE_EXPERIMENT, "composition order moved the landmark", [{"artifact_type": "metric"}]),
    (STAGE_FAILURE, "X @ A on square A applied the transpose; repaired as X @ A.T", [{"artifact_type": "table"}]),
    (STAGE_TRANSFER, "fresh scale-shear-rotate batch without assistance", [{"artifact_type": "artifact"}]),
    (STAGE_ADR, "kept column-vector convention with explicit batch transpose", [{"artifact_type": "markdown"}]),
    (STAGE_FLAGSHIP, "V04 dense layer is X @ W + b agreeing with A = W.T", [{"artifact_type": "markdown"}]),
...
@pytest.fixture
def m16_env(monkeypatch):
    fastapi = pytest.importorskip("fastapi")
    del fastapi
    home = Path(tempfile.mkdtemp(prefix="los-g6-m16-home-", dir="/tmp"))
    sock = Path(f"/tmp/los-g6-m16-{uuid.uuid4().hex}.sock")
...
def test_sample_result_matches_frozen_wp137() -> None:
    ...
    sample = {
        "execution_id": "exec-m16-sample",
        "status": "SUCCESS",
        "exit_code": 0,
        "duration_ms": 12,
        "blocks": [

            {"type": "metric", "title": "composition_order", "payload": {"order_matters": True}},
            {"type": "table", "title": "landmarks", "payload": {"columns": ["order", "x", "y"], "rows": [["shear_then_rotate", 0.0, 1.0]]}},
        ],
    }
...
def test_m16_runtime_roundtrip(m16_env):
    from fastapi.testclient import TestClient

    from app.main import app

    before = _home_mtime()
    home: Path = m16_env["home"]
    sock: Path = m16_env["sock"]
    env: dict[str, str] = m16_env["env"]
    ...
    try:
        ...
        with TestClient(app, client=("127.0.0.1", 50000)) as client:
            ...
            loaded = client.post(
                ...
                json={"package_dir": str(M16_PACKAGE)},
                headers=headers,
            )
            assert loaded.status_code == 200, loaded.text
            loaded_body = loaded.json()
            assert loaded_body.get("id") == "g6.reference.M16"
            ...
            assert "M16" in mission_ids
            assert "g6.reference.M16" not in mission_ids

            mission = client.get("/api/v1/missions/M16", headers=headers)
            assert mission.status_code == 200, mission.text
            spec = mission.json()
            assert spec.get("id") == "M16"
            ...
            learner = client.post(
                "/api/v1/learners",
                json={"username": "m16-learner", "display_name": "M16 Learner"},
                headers=headers,
            )
            ...
            session = client.post(
                "/api/v1/sessions",
                json={"mission_id": "M16", "learner_id": learner_id},
                headers=headers,
            )
            assert session.status_code == 200, session.text
            session_body = session.json()
            assert session_body["mission_id"] == "M16"
        ...
        with TestClient(app, client=("127.0.0.1", 50000)) as resumed:
            ...
            assert "kn.m16.composition_order" in nodes
            ...
            for stage_id, explanation, artifacts in SUBMIT_PLAN:
                ...
                assert metric["order_matters"] is True
                assert metric["shear_then_rotate"] != metric["rotate_then_shear"]
            ...
            assert increments == {'comp.mat.transformation', 'comp.mat.composition', 'comp.mat.batch_orientation', 'comp.mat.unassisted_transfer'}
