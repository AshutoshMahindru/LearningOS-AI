"""G6 M16 reference package: WP-136 spec, WP-137 rubric, generic runtime only."""
...
LANE_ALLOWED = tuple(
    f"platform/fixtures/M{i:02d}/" for i in range(15, 21)
) + tuple(
    f"tests/platform/reference/M{i:02d}/" for i in range(15, 21)
)


def _mission() -> dict:
    return json.loads((M16_PACKAGE / "missions" / "M16.json").read_text(encoding="utf-8"))


def test_package_identity_is_g6_reference() -> None:
    package = load_package(M16_PACKAGE)
...
def test_wp136_id_is_mission_not_package_id() -> None:
    ...
    assert mission["order_index"] == 16
    assert mission["flagship_version"] == "V04"
    assert mission["prerequisites"] == ["M15"]
...
def test_package_validates_against_wp136_and_g4_journey() -> None:
    result = validate_package(M16_PACKAGE)
...
def test_mdl_validator_accepts_sealed_package() -> None:
    ...
    payload = validate_mission(M16_PACKAGE / "missions" / "M16.json")
    assert payload["id"] == MISSION_ID
    package = mdl_validate_package(M16_PACKAGE)
    assert package.id == PACKAGE_ID
    loaded = g3_load(M16_PACKAGE)
...
def test_spec_and_rubric_use_wp137_block_types() -> None:
    ...
    assert "matrix" in blob
    for token in ('composition', 'transpose', 'shear'):
...
def test_experiment_is_generic_runner_with_assistance_ladder() -> None:
    ...
    assert runner["module"] == "g6.reference.M16.experiment"
    assert runner["entrypoint"] == "run_composition_order"
...
def test_gate_contract_evidence_and_targeted_repair() -> None:
    ...
    assert comps == {'comp.mat.transformation', 'comp.mat.composition', 'comp.mat.batch_orientation', 'comp.mat.unassisted_transfer'}
    for node in ('kn.m16.matrix_as_function', 'kn.m16.composition_order', 'kn.m16.batch_transpose', 'kn.m16.no_ai_transfer'):
        assert node in mission["knowledge_nodes"]


def test_package_has_no_prefilled_learner_evidence() -> None:
    blob = (M16_PACKAGE / "missions" / "M16.json").read_text(encoding="utf-8").lower()
    for forbidden in (
        "learner_evidence",
        "learner_response",
        "fabricated learner",
...
def test_package_has_no_custom_frontend_or_eager_pandas() -> None:
    ...
    assert f"/missions/m16/" not in mission
...
def test_platform_has_no_mission_special_case_routes_or_conditionals() -> None:
    ...
    eq_mid = re.compile(r"""mission_id\s*==\s*["']M16["']""")
    custom_route = re.compile(r"/m16[-_/]|matrix-transformations|batch-orientation", re.IGNORECASE)
    hits: list[str] = []
...
def _changed_paths() -> list[str]:
    ...
    if any(path.startswith("platform/fixtures/M16/") for path in committed):
        return committed
    extra = [
        path
...
def test_allowed_diff_paths_are_fixtures_and_reference_tests_only() -> None:
    ...
    assert any(path.startswith("platform/fixtures/M16/") for path in paths)
    assert any(path.startswith("tests/platform/reference/M16/") for path in paths)


def test_fixture_lives_only_in_allowed_g6_paths() -> None:
    relative = M16_PACKAGE.relative_to(REPO_ROOT)
    assert relative == Path("platform/fixtures/M16")
    tests = REPO_ROOT / "tests" / "platform" / "reference" / "M16"
    assert tests.is_dir()
