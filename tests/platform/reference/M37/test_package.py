"""G5 M37 reference package: WP-136 spec, WP-137 rubric, generic runtime only."""
...
SOURCE_TOKENS = ['predict', 'schema', 'idempotency', 'tool', 'v10', 'no custom', 'approval', 'retry', 'execution']


def _mission() -> dict:
    return json.loads((M37_PACKAGE / "missions" / "M37.json").read_text(encoding="utf-8"))


def test_package_identity_is_g5_reference() -> None:
    package = load_package(M37_PACKAGE)
...
def test_wp136_id_is_mission_not_package_id() -> None:
    ...
    assert mission["order_index"] == 37
    assert mission["flagship_version"] == "V10"
    assert mission["phase"]["id"] == "P7"
    assert mission["title"] == 'Make an LLM Call Tools Reliably'
    assert mission["prerequisites"] == ['M32']
...
def test_package_validates_against_wp136_and_g4_journey() -> None:
    result = validate_package(M37_PACKAGE)
...
def test_mdl_validator_accepts_sealed_package() -> None:
    ...
    payload = validate_mission(M37_PACKAGE / "missions" / "M37.json")
    assert payload["id"] == MISSION_ID
    package = mdl_validate_package(M37_PACKAGE)
    assert package.id == PACKAGE_ID
    loaded = g3_load(M37_PACKAGE)
...
def test_experiment_is_generic_runner_with_assistance_ladder() -> None:
    ...
    assert runner["module"] == "g5.reference.M37.experiment"
    assert runner["entrypoint"] == 'run_tool_validation'
...
def test_gate_contract_evidence_and_targeted_repair() -> None:
    ...
    assert comps == {'comp.tool.schema', 'comp.tool.idempotency', 'comp.tool.typed_errors', 'comp.tool.unassisted_transfer'}
    for node in ['kn.m37.schema', 'kn.m37.idempotency', 'kn.m37.typed_errors', 'kn.m37.no_ai_transfer']:
...
def test_package_has_no_prefilled_learner_evidence() -> None:
    blob = (M37_PACKAGE / "missions" / "M37.json").read_text(encoding="utf-8").lower()
    for forbidden in (
        "learner_evidence",
        "learner_response",
        "fabricated learner",
...
def test_platform_has_no_mission_special_case_routes_or_conditionals() -> None:
    ...
    eq_mid = re.compile(r"""mission_id\s*==\s*["']M37["']""")
    custom_route = re.compile(r"/m37[-_/]|tool-calling-runtime|teaching-registry", re.IGNORECASE)
    hits: list[str] = []
...
def test_frontend_registry_stays_generic() -> None:
    ...
    assert "M37" not in registry
    assert 'tool_calling_console' not in registry.lower()
...
def test_allowed_diff_paths_are_fixtures_and_reference_tests_only() -> None:
    ...
    assert any(path.startswith("platform/fixtures/M37/") for path in paths)
    assert any(path.startswith("tests/platform/reference/M37/") for path in paths)


def test_fixture_lives_only_in_allowed_g6_paths() -> None:
    relative = M37_PACKAGE.relative_to(REPO_ROOT)
    assert relative == Path("platform/fixtures/M37")
    tests = REPO_ROOT / "tests" / "platform" / "reference" / "M37"
    assert tests.is_dir()
