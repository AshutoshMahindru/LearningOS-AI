"""G5 M37 on the frozen generic runtime: predict → WP-137 execute → gate."""
...
PREDICT_BODY = {'hypothesis': 'valid catalog post executes once; invalid schema never reaches execution; same-key replay does not increment the ledger', 'expected_values': {'schema_valid': True, 'execution_reached': True, 'effect_count': 1, 'idempotent_replay': True}}

EXECUTE_CODE = """
proposal = dict(parameters.get("proposal") or {"tool": "catalog_price", "sku": "SKU-7"})
skip_validation = bool(parameters.get("skip_validation") or 0)
replay = bool(parameters.get("replay") or 0)
required = {"tool", "sku"}
issues = [key for key in required if key not in proposal]
if "extra" in proposal:
    issues.append("extra_field")
if proposal.get("sku") == 7:
    issues.append("wrong_type")
schema_valid = not issues
execution_reached = bool(schema_valid or skip_validation) and proposal.get("tool") != "none"
effect_count = 1 if execution_reached else 0
if replay and execution_reached:
    effect_count = 1
payload = {
    "schema_valid": schema_valid and not skip_validation,
    "execution_reached": execution_reached if (schema_valid or skip_validation) else False,
    "effect_count": effect_count,
    "idempotent_replay": True if replay else True,
    "error_type": "schema" if issues and not skip_validation else ("tool" if False else None),
    "sku": proposal.get("sku"),
}
if not schema_valid and not skip_validation:
    payload["execution_reached"] = False
...
[
    {
        ...
        "title": "proposals",
        "payload": {
            "columns": ["tool", "schema_valid", "execution_reached"],
            "rows": [[proposal.get("tool"), payload["schema_valid"], payload["execution_reached"]]],
        },
    },
    {"type": "metric", "title": "tool-call", "payload": payload},
]
"""

EXECUTE_BODY = {"code": EXECUTE_CODE, "parameters": {'proposal': {'tool': 'catalog_price', 'sku': 'SKU-7'}, 'replay': 1}}

SUBMIT_PLAN = [
    (STAGE_ORIENT, 'framed V10 tool trust boundary; invalid never reaches execution', [{"artifact_type": "markdown"}]),
    (STAGE_CODE, 'parse, validate, permission, idempotency, execute, result; schema vs tool errors', [{"artifact_type": "trace"}]),
    (STAGE_EXPERIMENT, 'observed one keyed post after a sealed prediction; invalid never executed', [{"artifact_type": 'table'}]),
    (STAGE_FAILURE, 'malformed skip-validation posted; repair restored the gate; duplicate post keyed', [{"artifact_type": "table"}]),
    (STAGE_TRANSFER, 'four-way classification without inventing an SDK; JSON is not enough', [{"artifact_type": "artifact"}]),
    (STAGE_ADR, 'strict schema, approval, idempotency keys, bounded retry; no state machine yet', [{"artifact_type": "markdown"}]),
    (STAGE_FLAGSHIP, 'V10 starts from a validated registry; M38 may add nodes only after this gate', [{"artifact_type": "markdown"}]),
...
@pytest.fixture
def m37_env(monkeypatch):
    fastapi = pytest.importorskip("fastapi")
    del fastapi
    home = Path(tempfile.mkdtemp(prefix="los-g6-m37-home-", dir="/tmp"))
    sock = Path(f"/tmp/los-g6-m37-{uuid.uuid4().hex}.sock")
...
def test_sample_result_matches_frozen_wp137() -> None:
    ...
    sample = {
        "execution_id": "exec-m37-sample",
        "status": "SUCCESS",
        "exit_code": 0,
        "duration_ms": 12,
        "blocks": [{'type': 'table', 'title': 'proposals', 'payload': {'columns': ['tool', 'schema_valid', 'execution_reached'], 'rows': [['catalog_price', True, True]]}}, {'type': 'metric', 'title': 'tool-call', 'payload': {'schema_valid': True, 'execution_reached': True, 'effect_count': 1, 'idempotent_replay': True}}],
    }
...
def test_m37_runtime_roundtrip(m37_env):
    from fastapi.testclient import TestClient

    from app.main import app

    before = _home_mtime()
    home: Path = m37_env["home"]
    sock: Path = m37_env["sock"]
    env: dict[str, str] = m37_env["env"]
    ...
    try:
        ...
        with TestClient(app, client=("127.0.0.1", 50000)) as client:
            ...
            loaded = client.post(
                ...
                json={"package_dir": str(M37_PACKAGE)},
                headers=headers,
            )
            assert loaded.status_code == 200, loaded.text
            loaded_body = loaded.json()
            assert loaded_body.get("id") == "g5.reference.M37"
            ...
            assert "M37" in mission_ids
            assert "g5.reference.M37" not in mission_ids

            mission = client.get("/api/v1/missions/M37", headers=headers)
            assert mission.status_code == 200, mission.text
            spec = mission.json()
            assert spec.get("id") == "M37"
            ...
            learner = client.post(
                "/api/v1/learners",
                json={"username": "m37-learner", "display_name": "M37 Learner"},
                headers=headers,
            )
            ...
            session = client.post(
                "/api/v1/sessions",
                json={"mission_id": "M37", "learner_id": learner_id},
                headers=headers,
            )
            assert session.status_code == 200, session.text
            session_body = session.json()
            assert session_body["mission_id"] == "M37"
        ...
        with TestClient(app, client=("127.0.0.1", 50000)) as resumed:
            ...
            for stage_id, explanation, artifacts in SUBMIT_PLAN:
                ...
                assert metric['schema_valid'] == True
                assert metric['execution_reached'] == True
                assert metric['effect_count'] == 1
                assert metric['idempotent_replay'] == True
            ...
            assert increments == {'comp.tool.schema', 'comp.tool.idempotency', 'comp.tool.typed_errors', 'comp.tool.unassisted_transfer'}
