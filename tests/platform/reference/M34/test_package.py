"""G5 M34 reference package: WP-136 spec, WP-137 rubric, generic runtime only."""
...
SOURCE_TOKENS = ['predict', 'citation', 'abstention', 'retrieval', 'support', 'v09', 'no custom', 'budget', 'provenance']


def _mission() -> dict:
    return json.loads((M34_PACKAGE / "missions" / "M34.json").read_text(encoding="utf-8"))


def test_package_identity_is_g5_reference() -> None:
    package = load_package(M34_PACKAGE)
...
def test_wp136_id_is_mission_not_package_id() -> None:
    ...
    assert mission["order_index"] == 34
    assert mission["flagship_version"] == "V09"
    assert mission["phase"]["id"] == "P6"
    assert mission["title"] == 'Build RAG'
    assert mission["prerequisites"] == ['M31', 'M33']
...
def test_package_validates_against_wp136_and_g4_journey() -> None:
    result = validate_package(M34_PACKAGE)
...
def test_mdl_validator_accepts_sealed_package() -> None:
    ...
    payload = validate_mission(M34_PACKAGE / "missions" / "M34.json")
    assert payload["id"] == MISSION_ID
    package = mdl_validate_package(M34_PACKAGE)
    assert package.id == PACKAGE_ID
    loaded = g3_load(M34_PACKAGE)
...
def test_experiment_is_generic_runner_with_assistance_ladder() -> None:
    ...
    assert runner["module"] == "g5.reference.M34.experiment"
    assert runner["entrypoint"] == 'run_grounded_rag'
...
def test_gate_contract_evidence_and_targeted_repair() -> None:
    ...
    assert comps == {'comp.rag.grounding', 'comp.rag.citations', 'comp.rag.abstention', 'comp.rag.unassisted_transfer'}
    for node in ['kn.m34.grounding', 'kn.m34.citations', 'kn.m34.abstention', 'kn.m34.no_ai_transfer']:
...
def test_package_has_no_prefilled_learner_evidence() -> None:
    blob = (M34_PACKAGE / "missions" / "M34.json").read_text(encoding="utf-8").lower()
    for forbidden in (
        "learner_evidence",
        "learner_response",
        "fabricated learner",
...
def test_platform_has_no_mission_special_case_routes_or_conditionals() -> None:
    ...
    eq_mid = re.compile(r"""mission_id\s*==\s*["']M34["']""")
    custom_route = re.compile(r"/m34[-_/]|grounded-rag|extractive-span", re.IGNORECASE)
    hits: list[str] = []
...
def test_frontend_registry_stays_generic() -> None:
    ...
    assert "M34" not in registry
    assert 'grounded_rag_pipeline' not in registry.lower()
...
def test_allowed_diff_paths_are_fixtures_and_reference_tests_only() -> None:
    ...
    assert any(path.startswith("platform/fixtures/M34/") for path in paths)
    assert any(path.startswith("tests/platform/reference/M34/") for path in paths)


def test_fixture_lives_only_in_allowed_g6_paths() -> None:
    relative = M34_PACKAGE.relative_to(REPO_ROOT)
    assert relative == Path("platform/fixtures/M34")
    tests = REPO_ROOT / "tests" / "platform" / "reference" / "M34"
    assert tests.is_dir()
