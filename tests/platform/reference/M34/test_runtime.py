"""G5 M34 on the frozen generic runtime: predict → WP-137 execute → gate."""
...
PREDICT_BODY = {'hypothesis': 'retrieval-enabled extractive copy is supported with a gold citation; closed-book and unanswerable paths abstain; a wrong citation fails support', 'expected_values': {'supported': True, 'abstained': False, 'eval_pass': True}}

EXECUTE_CODE = """
query = str(parameters.get("query") or "reset password")
top_k = int(parameters.get("top_k") or 3)
budget = int(parameters.get("budget_chars") or 400)
defect = str(parameters.get("defect") or "")
unanswerable = bool(parameters.get("unanswerable") or 0)
hits = [
    ("doc-account-access::c0", "Reset your password from the account page.", 0.91),
    ("doc-account-access::c1", "Use the login password reset flow.", 0.84),
    ("doc-device-printer::c0", "Please reset the printer firmware.", 0.41),
][:top_k]
packed = []
used = 0
for cid, text, score in hits:
    if used + len(text) > budget:
        continue
    packed.append({"chunk_id": cid, "text": text, "score": score})
    used += len(text)
gold = "doc-account-access::c0"
retrieval_hit = any(item["chunk_id"] == gold for item in packed)
if unanswerable:
    answer, citation, abstained = "", None, True
elif defect == "wrong_citation":
    answer, citation, abstained = "Reset your password from the account page.", "doc-device-printer::c0", False
elif not retrieval_hit:
    answer, citation, abstained = "", None, True
else:
    answer, citation, abstained = packed[0]["text"], packed[0]["chunk_id"], False
supported = bool(citation and any(item["chunk_id"] == citation and item["text"] in (answer or " ") for item in packed))
if citation and citation != gold:
    supported = False
eval_pass = supported and not abstained if not unanswerable else abstained
payload = {
    "supported": supported,
    "abstained": abstained,
...
print(payload)
[
    {"type": "metric", "title": "grounding", "payload": payload},
    {
        "type": "table",
        "title": "context-pack",
        "payload": {
            "columns": ["chunk_id", "chars"],
            "rows": [[item["chunk_id"], len(item["text"])] for item in packed],
        },
    },
]
"""

EXECUTE_BODY = {"code": EXECUTE_CODE, "parameters": {'top_k': 3, 'budget_chars': 400, 'unanswerable': 0}}

SUBMIT_PLAN = [
    (STAGE_ORIENT, 'framed V09 extractive RAG whole; gate needs support, abstention, citation fail-closed', [{"artifact_type": "markdown"}]),
    (STAGE_CODE, 'retrieve, pack, synthesize, cite, abstain, evaluate; IDs survive; tail-drop packing', [{"artifact_type": "trace"}]),
    (STAGE_EXPERIMENT, 'observed supported extractive answer after a sealed prediction', [{"artifact_type": 'metric'}]),
    (STAGE_FAILURE, 'fluent neighbor citation failed support; rebind-or-abstain from the broken pack', [{"artifact_type": "table"}]),
    (STAGE_TRANSFER, 'classified layers without inventing a live model; unsupported fluent text rejected', [{"artifact_type": "artifact"}]),
    (STAGE_ADR, 'support_gated extractive copy; unsupported claims block release', [{"artifact_type": "markdown"}]),
    (STAGE_FLAGSHIP, 'V09 starts from a frozen grounded baseline; M35 may not rewrite holdout ids', [{"artifact_type": "markdown"}]),
...
@pytest.fixture
def m34_env(monkeypatch):
    fastapi = pytest.importorskip("fastapi")
    del fastapi
    home = Path(tempfile.mkdtemp(prefix="los-g6-m34-home-", dir="/tmp"))
    sock = Path(f"/tmp/los-g6-m34-{uuid.uuid4().hex}.sock")
...
def test_sample_result_matches_frozen_wp137() -> None:
    ...
    sample = {
        "execution_id": "exec-m34-sample",
        "status": "SUCCESS",
        "exit_code": 0,
        "duration_ms": 12,
        "blocks": [{'type': 'metric', 'title': 'grounding', 'payload': {'supported': True, 'abstained': False, 'eval_pass': True}}, {'type': 'table', 'title': 'context-pack', 'payload': {'columns': ['chunk_id', 'chars'], 'rows': [['doc-account-access::c0', 40]]}}],
    }
...
def test_m34_runtime_roundtrip(m34_env):
    from fastapi.testclient import TestClient

    from app.main import app

    before = _home_mtime()
    home: Path = m34_env["home"]
    sock: Path = m34_env["sock"]
    env: dict[str, str] = m34_env["env"]
    ...
    try:
        ...
        with TestClient(app, client=("127.0.0.1", 50000)) as client:
            ...
            loaded = client.post(
                ...
                json={"package_dir": str(M34_PACKAGE)},
                headers=headers,
            )
            assert loaded.status_code == 200, loaded.text
            loaded_body = loaded.json()
            assert loaded_body.get("id") == "g5.reference.M34"
            ...
            assert "M34" in mission_ids
            assert "g5.reference.M34" not in mission_ids

            mission = client.get("/api/v1/missions/M34", headers=headers)
            assert mission.status_code == 200, mission.text
            spec = mission.json()
            assert spec.get("id") == "M34"
            ...
            learner = client.post(
                "/api/v1/learners",
                json={"username": "m34-learner", "display_name": "M34 Learner"},
                headers=headers,
            )
            ...
            session = client.post(
                "/api/v1/sessions",
                json={"mission_id": "M34", "learner_id": learner_id},
                headers=headers,
            )
            assert session.status_code == 200, session.text
            session_body = session.json()
            assert session_body["mission_id"] == "M34"
        ...
        with TestClient(app, client=("127.0.0.1", 50000)) as resumed:
            ...
            assert 'kn.m34.citations' in nodes
            ...
            for stage_id, explanation, artifacts in SUBMIT_PLAN:
                ...
                assert metric['supported'] == True
                assert metric['abstained'] == False
                assert metric['eval_pass'] == True
            ...
            assert increments == {'comp.rag.grounding', 'comp.rag.citations', 'comp.rag.abstention', 'comp.rag.unassisted_transfer'}
