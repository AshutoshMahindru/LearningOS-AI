"""G5 M27 reference package: WP-136 spec, WP-137 rubric, generic runtime only."""
...
def _mission() -> dict:
    return json.loads((M27_PACKAGE / "missions" / "M27.json").read_text(encoding="utf-8"))


def test_package_identity_is_g5_reference_m27() -> None:
    package = load_package(M27_PACKAGE)
...
def test_wp136_id_is_m27_not_package_id() -> None:
    mission = _mission()
    assert mission["id"] == MISSION_ID
    assert mission["id"] != PACKAGE_ID
    assert mission["order_index"] == 27
...
def test_package_validates_against_wp136_and_g4_journey() -> None:
    result = validate_package(M27_PACKAGE)
...
def test_mdl_validator_accepts_sealed_package() -> None:
    ...
    payload = validate_mission(M27_PACKAGE / "missions" / "M27.json")
    assert payload["id"] == MISSION_ID
    package = mdl_validate_package(M27_PACKAGE)
    assert package.id == PACKAGE_ID
    loaded = g3_load(M27_PACKAGE)
...
def test_experiment_is_catalogue_oriented_on_generic_runner() -> None:
    ...
    assert runner["module"] == "g5.reference.M27.experiment"
    assert runner["entrypoint"] == "run_tokenization"
...
def test_gate_contract_evidence_and_targeted_repair() -> None:
    ...
    assert set(comps) == set(["comp.tok.pieces_and_ids", "comp.tok.specials_and_budget", "comp.tok.scheme_comparison", "comp.tok.budget_failure_repair"])
    for node in ["kn.m27.tokenizer_identity", "kn.m27.specials_pad_truncate", "kn.m27.word_vs_bpe", "kn.m27.token_budget"]:
...
def test_package_has_no_prefilled_learner_evidence() -> None:
    blob = (M27_PACKAGE / "missions" / "M27.json").read_text(encoding="utf-8").lower()
    for forbidden in (
        "learner_evidence",
        "learner_response",
        "fabricated learner",
...
def test_source_mission_content_is_represented() -> None:
    ...
    for token in ["predict", "token", "bpe", "[bos]", "[eos]", "pad", "truncat", "no custom", "v06", "word"]:
        assert token in mission, token


def test_package_has_no_custom_frontend_or_runtime_payload() -> None:
    for path in M27_PACKAGE.rglob("*"):
        if not path.is_file():
            continue
...
def test_platform_has_no_m27_special_case_routes_or_conditionals() -> None:
    platform = REPO_ROOT / "platform"
    skip_dirs = {"node_modules", "dist", "__pycache__", ".git"}
    eq_mid = re.compile(r"""mission_id\s*==\s*["']M27["']""")
    custom_route = re.compile(r"/m27[-_/]|text-to-tokens|teaching-tokenizer", re.IGNORECASE)
    hits: list[str] = []
...
def test_allowed_diff_paths_are_fixtures_and_reference_tests_only() -> None:
    ...
    assert any(path.startswith("platform/fixtures/M27/") for path in paths)
    assert any(path.startswith("tests/platform/reference/M27/") for path in paths)
