"""G5 M27 on the frozen generic runtime: predict → WP-137 execute → gate."""
...
M27_PACKAGE = REPO_ROOT / "platform" / "fixtures" / "M27"
...
PREDICT_BODY = {"hypothesis": "the cat sat on the mat is in-vocab under word lookup; [BOS]/[EOS] add two to length; a word-budget can report fit while BPE drops approve_refund", "expected_values": {"content_length": 6, "effective_length": 8, "tokenizer_version": "v06.1"}}

EXECUTE_CODE = "VOCAB = {\"[PAD]\": 0, \"[UNK]\": 1, \"[BOS]\": 2, \"[EOS]\": 3, \"the\": 4, \"cat\": 5, \"sat\": 6, \"on\": 7, \"mat\": 8}\ntext = \"the cat sat on the mat\"\npieces = text.split()\nids = [VOCAB.get(p, 1) for p in pieces]\nwith_specials = [2] + ids + [3]\ncontent_length = len(ids)\neffective_length = len(with_specials)\nmax_tokens = int(parameters.get(\"max_tokens\") or 16)\ntruncated = with_specials[:max_tokens]\npad_to = int(parameters.get(\"pad_to\") or effective_length)\npadded = truncated + [0] * max(0, pad_to - len(truncated))\nmask = [1 if tok != 0 else 0 for tok in padded]\nid_to_piece = {v: k for k, v in VOCAB.items()}\nrows = [[id_to_piece.get(i, \"[UNK]\"), i] for i in padded]\nrare = \"approve_refund\"\nword_unk = VOCAB.get(rare, 1) == 1\nbpe_pieces = [\"approve\", \"_\", \"ref\", \"und\"]\npayload = {\n    \"tokenizer_family\": \"v06-teaching-tokenizer\",\n    \"tokenizer_version\": \"v06.1\",\n    \"content_length\": content_length,\n    \"effective_length\": effective_length,\n    \"max_tokens\": max_tokens,\n    \"word_unk_on_rare\": word_unk,\n    \"bpe_piece_count\": len(bpe_pieces),\n    \"downloaded\": False,\n}\nprint(payload)\n[\n    {\"type\": \"table\", \"title\": \"encode-trace\", \"payload\": {\"columns\": [\"piece\", \"id\"], \"rows\": rows}},\n    {\"type\": \"metric\", \"title\": \"budget\", \"payload\": payload},\n    {\"type\": \"artifact\", \"title\": \"rare-string\", \"payload\": {\"word_unk\": word_unk, \"bpe_pieces\": bpe_pieces, \"mask\": mask}},\n]"

EXECUTE_BODY = {
    "code": EXECUTE_CODE,
    "parameters": {"max_tokens": 16, "pad_to": 8},
}

SUBMIT_PLAN = [
    (STAGE_ORIENT, "framed text\u2192pieces\u2192IDs with v06.1 specials and token budgets", [{"artifact_type": "markdown"}]),
    (STAGE_EXPERIMENT, "observed WP-137 token table after a sealed prediction", [{"artifact_type": "table"}]),
    (STAGE_CODE, "traced normalize, pretokens, specials, truncate, pad, mask, identity", [{"artifact_type": "artifact"}]),
    (STAGE_FAILURE, "word budget silently dropped approve_refund; restore token budget", [{"artifact_type": "table"}]),
    (STAGE_TRANSFER, "fresh BPE trace, effective length, truncation diagnosis without AI", [{"artifact_type": "metric"}]),
    (STAGE_ADR, "canonicalized v06-teaching-bpe v06.1; rejected character budgets", [{"artifact_type": "markdown"}]),
    (STAGE_FLAGSHIP, "V06 starts from a named tokenizer, version, and token budget", [{"artifact_type": "markdown"}]),
...
@pytest.fixture
def m27_env(monkeypatch):
    home = Path(tempfile.mkdtemp(prefix="los-g6-m27-home-", dir="/tmp"))
    sock = Path(f"/tmp/los-g6-m27-{uuid.uuid4().hex}.sock")
...
def test_sample_result_matches_frozen_wp137() -> None:
    ...
    sample = {
        "execution_id": "exec-m27-ref",
        "status": "SUCCESS",
        "exit_code": 0,
        "duration_ms": 12,
        "blocks": [{"type": "table", "title": "encode-trace", "payload": {"columns": ["piece", "id"], "rows": [["[BOS]", 2], ["the", 4], ["cat", 5]]}}, {"type": "metric", "title": "budget", "payload": {"content_length": 6, "effective_length": 8, "tokenizer_version": "v06.1"}}],
    }
...
def test_m27_runtime_roundtrip_emits_wp137(m27_env):
    from fastapi.testclient import TestClient

    from app.main import app

    home: Path = m27_env["home"]
    sock: Path = m27_env["sock"]
    env: dict[str, str] = m27_env["env"]
    ...
    try:
        ...
        with TestClient(app, client=("127.0.0.1", 50000)) as client:
            ...
            loaded = client.post(
                ...
                json={"package_dir": str(M27_PACKAGE)},
                headers=headers,
            )
            assert loaded.status_code == 200, loaded.text
            loaded_body = loaded.json()
            assert loaded_body.get("id") == "g5.reference.M27"
            ...
            assert "M27" in mission_ids
            assert "g5.reference.M27" not in mission_ids

            mission = client.get("/api/v1/missions/M27", headers=headers)
            assert mission.status_code == 200, mission.text
            spec = mission.json()
            assert spec.get("id") == "M27"
            ...
            learner = client.post(
                "/api/v1/learners",
                json={"username": "m27-learner", "display_name": "M27 Learner"},
                headers=headers,
            )
            ...
            session = client.post(
                "/api/v1/sessions",
                json={"mission_id": "M27", "learner_id": learner_id},
                headers=headers,
            )
            assert session.status_code == 200, session.text
            session_body = session.json()
            assert session_body["mission_id"] == "M27"
        ...
        with TestClient(app, client=("127.0.0.1", 50000)) as resumed:
            ...
            for stage_id, explanation, artifacts in SUBMIT_PLAN:
                ...
                assert table["payload"]["columns"][0] == "piece"
                metric = next(block for block in blocks if block["type"] == "metric")
                assert metric["payload"]["content_length"] == 6
                assert metric["payload"]["effective_length"] == 8
                assert metric["payload"]["tokenizer_version"] == "v06.1"
            ...
            assert {item["competency_id"] for item in increments} == {"comp.tok.pieces_and_ids", "comp.tok.specials_and_budget", "comp.tok.scheme_comparison", "comp.tok.budget_failure_repair"}
