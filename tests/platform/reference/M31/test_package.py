"""G5 M31 reference package: WP-136 spec, WP-137 rubric, generic runtime only."""
...
def _mission() -> dict:
    return json.loads((M31_PACKAGE / "missions" / "M31.json").read_text(encoding="utf-8"))


def test_package_identity_is_g5_reference_m31() -> None:
    package = load_package(M31_PACKAGE)
...
def test_wp136_id_is_m31_not_package_id() -> None:
    mission = _mission()
    assert mission["id"] == MISSION_ID
    assert mission["id"] != PACKAGE_ID
    assert mission["order_index"] == 31
    assert mission["flagship_version"] == "V07"
    assert mission["phase"]["id"] == "P5"
    assert mission["prerequisites"] == ["M30"]
...
def test_package_validates_against_wp136_and_g4_journey() -> None:
    result = validate_package(M31_PACKAGE)
...
def test_mdl_validator_accepts_sealed_package() -> None:
    ...
    payload = validate_mission(M31_PACKAGE / "missions" / "M31.json")
    assert payload["id"] == MISSION_ID
    package = mdl_validate_package(M31_PACKAGE)
    assert package.id == PACKAGE_ID
    loaded = g3_load(M31_PACKAGE)
...
def test_experiment_is_catalogue_oriented_on_generic_runner() -> None:
    ...
    assert runner["module"] == "g5.reference.M31.experiment"
    assert runner["entrypoint"] == "run_causal_lm_teaching"
...
def test_gate_contract_evidence_and_targeted_repair() -> None:
    ...
    assert set(comps) == set(["comp.lm.next_token_pairs", "comp.lm.train_eval_boundary", "comp.lm.contamination", "comp.lm.lifecycle_stages"])
    for node in ["kn.m31.shift_identity", "kn.m31.held_out_nll", "kn.m31.leak_e02", "kn.m31.stage_map"]:
...
def test_package_has_no_prefilled_learner_evidence() -> None:
    blob = (M31_PACKAGE / "missions" / "M31.json").read_text(encoding="utf-8").lower()
    for forbidden in (
        "learner_evidence",
        "learner_response",
        "fabricated learner",
...
def test_source_mission_content_is_represented() -> None:
    ...
    for token in ["predict", "next-token", "shift", "contamination", "nll", "no custom", "v07", "checkpoint"]:
        assert token in mission, token


def test_package_has_no_custom_frontend_or_runtime_payload() -> None:
    for path in M31_PACKAGE.rglob("*"):
        if not path.is_file():
            continue
...
def test_platform_has_no_m31_special_case_routes_or_conditionals() -> None:
    platform = REPO_ROOT / "platform"
    skip_dirs = {"node_modules", "dist", "__pycache__", ".git"}
    eq_mid = re.compile(r"""mission_id\s*==\s*["']M31["']""")
    custom_route = re.compile(r"/m31[-_/]|llm-training|bigram-score-table", re.IGNORECASE)
    hits: list[str] = []
...
def test_allowed_diff_paths_are_fixtures_and_reference_tests_only() -> None:
    ...
    assert any(path.startswith("platform/fixtures/M31/") for path in paths)
    assert any(path.startswith("tests/platform/reference/M31/") for path in paths)
