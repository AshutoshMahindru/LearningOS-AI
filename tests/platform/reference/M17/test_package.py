"""G6 M17 reference package: WP-136 spec, WP-137 rubric, generic runtime only."""
...
def _mission() -> dict:
    return json.loads((M17_PACKAGE / "missions" / "M17.json").read_text(encoding="utf-8"))


def test_package_identity_is_g6_reference() -> None:
    package = load_package(M17_PACKAGE)
...
def test_wp136_id_is_mission_not_package_id() -> None:
    ...
    assert mission["order_index"] == 17
    assert mission["flagship_version"] == "V04"
    assert mission["prerequisites"] == ["M09"]
...
def test_package_validates_against_wp136_and_g4_journey() -> None:
    result = validate_package(M17_PACKAGE)
...
def test_mdl_validator_accepts_sealed_package() -> None:
    ...
    payload = validate_mission(M17_PACKAGE / "missions" / "M17.json")
    assert payload["id"] == MISSION_ID
    package = mdl_validate_package(M17_PACKAGE)
    assert package.id == PACKAGE_ID
    loaded = g3_load(M17_PACKAGE)
...
def test_spec_and_rubric_use_wp137_block_types() -> None:
    ...
    assert "uncertain" in blob
    for token in ('base-rate', 'posterior', 'sensitivity'):
...
def test_experiment_is_generic_runner_with_assistance_ladder() -> None:
    ...
    assert runner["module"] == "g6.reference.M17.experiment"
    assert runner["entrypoint"] == "run_screening_counts"
...
def test_gate_contract_evidence_and_targeted_repair() -> None:
    ...
    assert comps == {'comp.prob.frequency', 'comp.prob.conditional', 'comp.prob.base_rate', 'comp.prob.unassisted_transfer'}
    for node in ('kn.m17.empirical_frequency', 'kn.m17.conditional_probability', 'kn.m17.base_rate_neglect', 'kn.m17.no_ai_transfer'):
        assert node in mission["knowledge_nodes"]


def test_package_has_no_prefilled_learner_evidence() -> None:
    blob = (M17_PACKAGE / "missions" / "M17.json").read_text(encoding="utf-8").lower()
    for forbidden in (
        "learner_evidence",
        "learner_response",
        "fabricated learner",
...
def test_package_has_no_custom_frontend_or_eager_pandas() -> None:
    ...
    assert f"/missions/m17/" not in mission
...
def test_platform_has_no_mission_special_case_routes_or_conditionals() -> None:
    ...
    eq_mid = re.compile(r"""mission_id\s*==\s*["']M17["']""")
    custom_route = re.compile(r"/m17[-_/]|probability-uncertainty|base-rate", re.IGNORECASE)
    hits: list[str] = []
...
def _changed_paths() -> list[str]:
    ...
    if any(path.startswith("platform/fixtures/M17/") for path in committed):
        return committed
    extra = [
        path
...
def test_allowed_diff_paths_are_fixtures_and_reference_tests_only() -> None:
    ...
    assert any(path.startswith("platform/fixtures/M17/") for path in paths)
    assert any(path.startswith("tests/platform/reference/M17/") for path in paths)


def test_fixture_lives_only_in_allowed_g6_paths() -> None:
    relative = M17_PACKAGE.relative_to(REPO_ROOT)
    assert relative == Path("platform/fixtures/M17")
    tests = REPO_ROOT / "tests" / "platform" / "reference" / "M17"
    assert tests.is_dir()
