"""G6 M06 on the frozen generic runtime: predict seal, no-AI, evidence, gate, repair."""
...
PACKAGE = REPO_ROOT / "platform" / "fixtures" / "M06"
...
PACKAGE_ID = "g6.reference.M06"
MISSION_ID = "M06"
EXPERIMENT_STAGE = "stage_05_experiment"
...
PREDICT_BODY = {"hypothesis": "truncated ylim exaggerates modest tenure gaps while group means stay unchanged", "expected_values": {"unchanged_values": True}}

EXECUTE_CODE = """
means = {'email': 18.2, 'chat': 19.1, 'phone': 21.4}
payload = {
    'means': means,
    'ylim_truncated': [17, 22],
    'ylim_full': [0, 50],
    'unchanged_values': True,
    'majority_share': 0.82,
}
print(payload)
[
    {
        'type': 'chart',
        'title': 'tenure_by_channel',
        'payload': {
            'chart_type': 'bar',
            'series': [{'name': 'mean_tenure', 'x': list(means), 'y': list(means.values())}],
        },
    },
    {'type': 'metric', 'title': 'scale', 'payload': payload},
    {
        'type': 'table',
        'title': 'group_rates',
        'payload': {'columns': ['channel', 'mean_tenure', 'n'], 'rows': [['email', 18.2, 40], ['chat', 19.1, 25], ['phone', 21.4, 15]]},
    },
]
"""

EXECUTE_BODY = {"code": EXECUTE_CODE, "parameters": {}}
EXPECTED_BLOCK = "chart"
REPAIR_NODE = "kn.m06.question_before_chart"
...
def env_home(monkeypatch):
    ...
    del fastapi
    home = Path(tempfile.mkdtemp(prefix="los-g6-m06-home-", dir="/tmp"))
    sock = Path(f"/tmp/los-g6-m06-{uuid.uuid4().hex}.sock")
...
def _load_and_start(client, headers):
    ...
    learner = client.post(
        "/api/v1/learners",
        json={"username": "m06-learner", "display_name": "M06 Learner"},
        headers=headers,
    )
