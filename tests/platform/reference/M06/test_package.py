...
MISSION_PATH = PACKAGE / "missions" / "M06.json"
...
FLAGSHIP = "V01"
PHASE_ID = "P1"
TITLE = "See the Data Before Modeling It"
FROZEN_STAGE_TYPES = {
    "orientation",
    "trace_map",
    "interrogate",
...
SOURCE_TOKENS = ["question", "chart", "truncated", "community", "observation", "inference", "post_case_priority", "v01"]
...
def test_package_layout_and_identity() -> None:
    ...
    assert manifest["missions"][0]["path"] == "missions/M06.json"
...
def test_integrity_files_match_payload() -> None:
    ...
    assert "missions/M06.json" in checksums
...
def test_prediction_cycle_on_every_experiment_stage() -> None:
    ...
    for stage in experiments:
        ...
        assert runner.get("module", "").startswith("g6.reference.M06.")
...
def test_package_has_no_custom_frontend_or_runtime_payload() -> None:
    ...
    mission = json.dumps(_mission()).lower()
    assert "/missions/m06/" not in mission
...
def test_platform_has_no_mission_special_case_routes() -> None:
    ...
    eq_mission = re.compile(r"""==\s*["']M06["']""")
    route_mission = re.compile(r"/missions/M06")
    hits: list[str] = []
    for root in roots:
        if not root.exists():
            continue
...
def test_fixture_lives_only_in_allowed_g6_paths() -> None:
    ...
    assert relative == Path("platform/fixtures/M06")
    tests = REPO_ROOT / "tests" / "platform" / "reference" / "M06"
    assert tests.is_dir()
