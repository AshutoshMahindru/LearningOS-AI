...
MISSION_PATH = PACKAGE / "missions" / "M08.json"
...
SOURCE_TOKENS = ["baseline", "mae", "rmse", "residual", "post_sale_assessment", "leakage", "housing", "v02"]
...
def test_package_layout_and_identity() -> None:
    ...
    assert manifest["missions"][0]["path"] == "missions/M08.json"
...
def test_integrity_files_match_payload() -> None:
    ...
    assert "missions/M08.json" in checksums
...
def test_prediction_cycle_on_every_experiment_stage() -> None:
    ...
    for stage in experiments:
        ...
        assert runner.get("module", "").startswith("g6.reference.M08.")
...
def test_package_has_no_custom_frontend_or_runtime_payload() -> None:
    ...
    mission = json.dumps(_mission()).lower()
    assert "/missions/m08/" not in mission
...
def test_platform_has_no_mission_special_case_routes() -> None:
    ...
    eq_mission = re.compile(r"""==\s*["']M08["']""")
    route_mission = re.compile(r"/missions/M08")
    hits: list[str] = []
    for root in roots:
        if not root.exists():
            continue
...
def test_fixture_lives_only_in_allowed_g6_paths() -> None:
    ...
    assert relative == Path("platform/fixtures/M08")
    tests = REPO_ROOT / "tests" / "platform" / "reference" / "M08"
    assert tests.is_dir()
