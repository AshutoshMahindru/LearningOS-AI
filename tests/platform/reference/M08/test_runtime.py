"""G6 M08 on the frozen generic runtime: predict seal, no-AI, evidence, gate, repair."""
...
PACKAGE = REPO_ROOT / "platform" / "fixtures" / "M08"
...
PREDICT_BODY = {"hypothesis": "leaky post-sale field collapses MAE while the safe model only modestly beats the mean baseline", "expected_values": {"leaky_better_than_safe": True}}
...
payload = {
    ...
    'train_r2': 0.99,
    'cv_mae': 8.1,
    'post_sale_forbidden': True,
}
print(payload)
[
    {'type': 'metric', 'title': 'regression', 'payload': payload},
    {
        'type': 'chart',
        'title': 'residuals',
        'payload': {'chart_type': 'scatter', 'series': [{'name': 'residual', 'x': [1, 2, 3], 'y': [0.4, -0.2, 0.1]}]},
    },
]
"""
...
REPAIR_NODE = "kn.m08.residuals_cv"
...
def env_home(monkeypatch):
    ...
    del fastapi
    home = Path(tempfile.mkdtemp(prefix="los-g6-m08-home-", dir="/tmp"))
    sock = Path(f"/tmp/los-g6-m08-{uuid.uuid4().hex}.sock")
...
def _load_and_start(client, headers):
    ...
    learner = client.post(
        "/api/v1/learners",
        json={"username": "m08-learner", "display_name": "M08 Learner"},
        headers=headers,
    )
