"""G6 M12 reference package: WP-136 spec on the frozen generic runtime."""
...
MISSION_PATH = M12_PACKAGE / "missions" / "M12.json"
PACKAGE_ID = "g5.reference.M12"
...
def test_package_identity_is_g5_reference_m12() -> None:
    package = load_package(M12_PACKAGE)
    assert package.id == PACKAGE_ID
    assert package.version == "5.0.0"
    assert package.missions[0]["id"] == "M12"
    assert re.match(r"^M[0-9]{2}$", package.missions[0]["id"])
    assert package.id != "M12"
...
def test_wp136_and_g4_journey_contract() -> None:
    result = validate_package(M12_PACKAGE)
    assert result.ok, result.errors
    assert result.package_id == PACKAGE_ID
    assert result.mission_ids == ["M12"]
    assert result.schema_engine in {"jsonschema", "mdl_validator"}
    mdl = validate_mdl_package(M12_PACKAGE)
    assert mdl.id == PACKAGE_ID
    spec = validate_mission(MISSION_PATH)
    assert spec["id"] == "M12"
    assert spec["order_index"] == 12
    assert spec["flagship_version"] == "V03"
    assert spec["prerequisites"] == ["M11"]


def test_g3_loader_accepts_sealed_package() -> None:
    package = g3_load(M12_PACKAGE)
    assert package.id == PACKAGE_ID
    assert package.missions[0]["id"] == "M12"
    assert package.digest == load_package(M12_PACKAGE).digest


def test_integrity_files_match_payload() -> None:
    checksums = (M12_PACKAGE / "SHA256SUMS").read_text(encoding="utf-8")
    for rel in (
        "manifest.json",
        "missions/M12.json",
        "g5/reference/M12/experiment.py",
        "g5/reference/M12/data/ensemble_fixture.csv",
    ):
        assert rel in checksums
        assert sha256_file(M12_PACKAGE / rel) in checksums
...
def test_stages_are_catalogue_types_with_prediction_cycle() -> None:
    ...
    assert runner.get("module") == "g5.reference.M12.experiment"
    assert runner.get("entrypoint") == "run_ensemble_comparison"
...
def test_lab_source_has_no_eager_pandas() -> None:
    source = (M12_PACKAGE / "g5" / "reference" / "M12" / "experiment.py").read_text(encoding="utf-8")
    ...
    assert "ENSEMBLE_EXECUTE_SOURCE" in source
...
def test_reference_tests_collect_without_pandas(tmp_path: Path) -> None:
    ...
    env["PYTHONPATH"] = os.pathsep.join(
        [str(tmp_path), str(M12_PACKAGE), str(REPO_ROOT), str(REPO_ROOT / "platform" / "backend")]
    )
    completed = subprocess.run(
        [sys.executable, "-m", "pytest", "--collect-only", "-q", "tests/platform/reference/M12"],
        cwd=str(REPO_ROOT),
        capture_output=True,
        text=True,
        check=False,
...
def test_package_has_no_custom_frontend() -> None:
    for path in M12_PACKAGE.rglob("*"):
        if path.is_file():
            assert path.suffix not in {".tsx", ".ts", ".jsx", ".js", ".css", ".html"}
    assert "bespoke" not in json.dumps(_mission()).lower()


def test_platform_has_no_m12_special_case_routes() -> None:
    roots = [REPO_ROOT / "platform" / "backend" / "app", REPO_ROOT / "platform" / "frontend" / "src"]
    eq_m12 = re.compile(r"""==\s*["']M12["']""")
    custom_route = re.compile(r"/m12[-_/]|/tree-ensembles", re.IGNORECASE)
    hits: list[str] = []
    for root in roots:
        if not root.exists():
            continue
        for path in root.rglob("*"):
            ...
            if eq_m12.search(text) or custom_route.search(text):
...
def test_allowed_diff_paths_are_fixtures_and_reference_tests_only() -> None:
    ...
    if other_missions:
        pytest.skip("combined G6 integration tree; exclusive-write bound to 51B M12-M14")
    assert any(path.startswith("platform/fixtures/M12/") for path in paths)
    assert any(path.startswith("tests/platform/reference/M12/") for path in paths)
