"""G5 M32 reference package: WP-136 spec, WP-137 rubric, generic runtime only."""
...
def _mission() -> dict:
    return json.loads((M32_PACKAGE / "missions" / "M32.json").read_text(encoding="utf-8"))


def test_package_identity_is_g5_reference_m32() -> None:
    package = load_package(M32_PACKAGE)
...
def test_package_validates_against_wp136_and_g4_journey() -> None:
    result = validate_package(M32_PACKAGE)
...
def test_mdl_validator_accepts_sealed_package() -> None:
    ...
    payload = validate_mission(M32_PACKAGE / "missions" / "M32.json")
    assert payload["id"] == MISSION_ID
    package = mdl_validate_package(M32_PACKAGE)
    assert package.id == PACKAGE_ID
    loaded = g3_load(M32_PACKAGE)
...
def test_experiment_is_catalogue_oriented_on_generic_runner() -> None:
    ...
    assert runner["module"] == "g5.reference.M32.experiment"
    assert runner["entrypoint"] == "run_inference_controls"
...
def test_gate_contract_evidence_and_targeted_repair() -> None:
    ...
    assert set(comps) == set(["comp.inf.greedy_temperature", "comp.inf.filters_and_seed", "comp.inf.config_evidence", "comp.inf.adaptation_hierarchy"])
    for node in ["kn.m32.argmax", "kn.m32.softmax_temp", "kn.m32.inference_config", "kn.m32.freshness_not_finetune"]:
...
def test_package_has_no_prefilled_learner_evidence() -> None:
    blob = (M32_PACKAGE / "missions" / "M32.json").read_text(encoding="utf-8").lower()
    for forbidden in (
        "learner_evidence",
        "learner_response",
        "fabricated learner",
...
def test_source_mission_content_is_represented() -> None:
    ...
    for token in ["predict", "temperature", "greedy", "top-k", "seed", "no custom", "v07", "adaptation"]:
        assert token in mission, token


def test_package_has_no_custom_frontend_or_runtime_payload() -> None:
    for path in M32_PACKAGE.rglob("*"):
        if not path.is_file():
            continue
...
def test_platform_has_no_m32_special_case_routes_or_conditionals() -> None:
    platform = REPO_ROOT / "platform"
    skip_dirs = {"node_modules", "dist", "__pycache__", ".git"}
    eq_mid = re.compile(r"""mission_id\s*==\s*["']M32["']""")
    custom_route = re.compile(r"/m32[-_/]|inference-adaptation|teaching-inference", re.IGNORECASE)
    hits: list[str] = []
...
def test_allowed_diff_paths_are_fixtures_and_reference_tests_only() -> None:
    ...
    assert any(path.startswith("platform/fixtures/M32/") for path in paths)
    assert any(path.startswith("tests/platform/reference/M32/") for path in paths)
