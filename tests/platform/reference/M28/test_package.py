"""G5 M28 reference package: WP-136 spec, WP-137 rubric, generic runtime only."""
...
def _mission() -> dict:
    return json.loads((M28_PACKAGE / "missions" / "M28.json").read_text(encoding="utf-8"))


def test_package_identity_is_g5_reference_m28() -> None:
    package = load_package(M28_PACKAGE)
...
def test_package_validates_against_wp136_and_g4_journey() -> None:
    result = validate_package(M28_PACKAGE)
...
def test_mdl_validator_accepts_sealed_package() -> None:
    ...
    payload = validate_mission(M28_PACKAGE / "missions" / "M28.json")
    assert payload["id"] == MISSION_ID
    package = mdl_validate_package(M28_PACKAGE)
    assert package.id == PACKAGE_ID
    loaded = g3_load(M28_PACKAGE)
...
def test_experiment_is_catalogue_oriented_on_generic_runner() -> None:
    ...
    assert runner["module"] == "g5.reference.M28.experiment"
    assert runner["entrypoint"] == "run_embedding_ranking"
...
def test_gate_contract_evidence_and_targeted_repair() -> None:
    ...
    assert set(comps) == set(["comp.emb.cosine_ranking", "comp.emb.lexical_vs_semantic", "comp.emb.provenance_gate", "comp.emb.hard_cases"])
    for node in ["kn.m28.cosine_neighbors", "kn.m28.jaccard_disagreement", "kn.m28.fingerprint", "kn.m28.negation_numeric"]:
...
def test_package_has_no_prefilled_learner_evidence() -> None:
    blob = (M28_PACKAGE / "missions" / "M28.json").read_text(encoding="utf-8").lower()
    for forbidden in (
        "learner_evidence",
        "learner_response",
        "fabricated learner",
...
def test_source_mission_content_is_represented() -> None:
    ...
    for token in ["predict", "cosine", "provenance", "paraphrase", "l2", "no custom", "v06", "jaccard"]:
        assert token in mission, token


def test_package_has_no_custom_frontend_or_runtime_payload() -> None:
    for path in M28_PACKAGE.rglob("*"):
        if not path.is_file():
            continue
...
def test_platform_has_no_m28_special_case_routes_or_conditionals() -> None:
    platform = REPO_ROOT / "platform"
    skip_dirs = {"node_modules", "dist", "__pycache__", ".git"}
    eq_mid = re.compile(r"""mission_id\s*==\s*["']M28["']""")
    custom_route = re.compile(r"/m28[-_/]|embedding-retrieval|teaching-meanpool", re.IGNORECASE)
    hits: list[str] = []
...
def test_allowed_diff_paths_are_fixtures_and_reference_tests_only() -> None:
    ...
    assert any(path.startswith("platform/fixtures/M28/") for path in paths)
    assert any(path.startswith("tests/platform/reference/M28/") for path in paths)
