"""G5 M28 on the frozen generic runtime: predict → WP-137 execute → gate."""
...
M28_PACKAGE = REPO_ROOT / "platform" / "fixtures" / "M28"
...
PREDICT_BODY = {"hypothesis": "login paraphrase ranks login above printer; Jaccard and cosine disagree on overlap traps", "expected_values": {"top_id": "login", "metric": "cosine", "normalization": "l2"}}

EXECUTE_CODE = "import math\n\ndef dot(a, b):\n    return sum(x * y for x, y in zip(a, b))\n\ndef norm(a):\n    return math.sqrt(dot(a, a))\n\ndef cosine(a, b):\n    return dot(a, b) / (norm(a) * norm(b))\n\nquery = [1.0, 0.0]\ncorpus = {\"login\": [0.98, 0.04], \"printer\": [0.12, 0.95], \"paraphrase\": [0.99, 0.02]}\nscores = {k: cosine(query, v) for k, v in corpus.items() if k != \"paraphrase\"}\nranked = sorted(scores.items(), key=lambda kv: (-kv[1], kv[0]))\ntop_id, top_score = ranked[0]\nmargin = ranked[0][1] - ranked[1][1]\nparaphrase = cosine(query, corpus[\"paraphrase\"])\npayload = {\n    \"top_id\": top_id,\n    \"top_score\": top_score,\n    \"margin\": margin,\n    \"metric\": \"cosine\",\n    \"normalization\": \"l2\",\n    \"model\": \"v06-teaching-meanpool\",\n    \"version\": \"v06.1\",\n    \"paraphrase_agrees\": paraphrase > scores[\"printer\"],\n}\nprint(payload)\n[\n    {\"type\": \"metric\", \"title\": \"ranking\", \"payload\": payload},\n    {\"type\": \"table\", \"title\": \"neighbors\", \"payload\": {\"columns\": [\"id\", \"cosine\"], \"rows\": [[k, v] for k, v in ranked]}},\n    {\"type\": \"artifact\", \"title\": \"fingerprint\", \"payload\": {\"model\": \"v06-teaching-meanpool\", \"version\": \"v06.1\", \"pooling\": \"mean\"}},\n]"
...
SUBMIT_PLAN = [
    (STAGE_ORIENT, "framed cosine-on-L2 with a mix-or-refuse provenance contract", [{"artifact_type": "markdown"}]),
    (STAGE_EXPERIMENT, "observed WP-137 cosine metric after a sealed prediction", [{"artifact_type": "metric"}]),
    (STAGE_CODE, "traced pool, L2, fingerprint, cosine, (-score, id) ties", [{"artifact_type": "artifact"}]),
    (STAGE_FAILURE, "v06.2 mix ranked a printer hit; restore the provenance gate", [{"artifact_type": "table"}]),
    (STAGE_TRANSFER, "hand cosine, one hit, one miss, mismatch fields, not proof", [{"artifact_type": "artifact"}]),
    (STAGE_ADR, "locked v06-teaching-meanpool v06.1 L2 cosine; refused silent mixes", [{"artifact_type": "markdown"}]),
    (STAGE_FLAGSHIP, "V06/V08 consume the fingerprint; M29 may not rewrite it", [{"artifact_type": "markdown"}]),
...
@pytest.fixture
def m28_env(monkeypatch):
    home = Path(tempfile.mkdtemp(prefix="los-g6-m28-home-", dir="/tmp"))
    sock = Path(f"/tmp/los-g6-m28-{uuid.uuid4().hex}.sock")
...
def test_sample_result_matches_frozen_wp137() -> None:
    ...
    sample = {
        "execution_id": "exec-m28-ref",
        "status": "SUCCESS",
        "exit_code": 0,
        "duration_ms": 12,
        "blocks": [{"type": "metric", "title": "ranking", "payload": {"top_id": "login", "metric": "cosine", "normalization": "l2"}}, {"type": "table", "title": "neighbors", "payload": {"columns": ["id", "cosine"], "rows": [["login", 0.99], ["printer", 0.1]]}}],
    }
...
def test_m28_runtime_roundtrip_emits_wp137(m28_env):
    from fastapi.testclient import TestClient

    from app.main import app

    home: Path = m28_env["home"]
    sock: Path = m28_env["sock"]
    env: dict[str, str] = m28_env["env"]
    ...
    try:
        ...
        with TestClient(app, client=("127.0.0.1", 50000)) as client:
            ...
            loaded = client.post(
                ...
                json={"package_dir": str(M28_PACKAGE)},
                headers=headers,
            )
            assert loaded.status_code == 200, loaded.text
            loaded_body = loaded.json()
            assert loaded_body.get("id") == "g5.reference.M28"
            ...
            assert "M28" in mission_ids
            assert "g5.reference.M28" not in mission_ids

            mission = client.get("/api/v1/missions/M28", headers=headers)
            assert mission.status_code == 200, mission.text
            spec = mission.json()
            assert spec.get("id") == "M28"
            ...
            learner = client.post(
                "/api/v1/learners",
                json={"username": "m28-learner", "display_name": "M28 Learner"},
                headers=headers,
            )
            ...
            session = client.post(
                "/api/v1/sessions",
                json={"mission_id": "M28", "learner_id": learner_id},
                headers=headers,
            )
            assert session.status_code == 200, session.text
            session_body = session.json()
            assert session_body["mission_id"] == "M28"
        ...
        with TestClient(app, client=("127.0.0.1", 50000)) as resumed:
            ...
            for stage_id, explanation, artifacts in SUBMIT_PLAN:
                ...
                assert metric["payload"]["top_id"] == "login"
                assert metric["payload"]["metric"] == "cosine"
                assert metric["payload"]["normalization"] == "l2"
                assert metric["payload"]["paraphrase_agrees"] is True
            ...
            assert {item["competency_id"] for item in increments} == {"comp.emb.cosine_ranking", "comp.emb.lexical_vs_semantic", "comp.emb.provenance_gate", "comp.emb.hard_cases"}
