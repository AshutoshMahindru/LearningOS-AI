"""G5 M39 reference package: WP-136 spec, WP-137 rubric, generic runtime only."""
...
SOURCE_TOKENS = ['predict', 'memory', 'routing', 'fallback', 'circuit', 'v10', 'no custom', 'degraded', 'provenance']


def _mission() -> dict:
    return json.loads((M39_PACKAGE / "missions" / "M39.json").read_text(encoding="utf-8"))


def test_package_identity_is_g5_reference() -> None:
    package = load_package(M39_PACKAGE)
...
def test_wp136_id_is_mission_not_package_id() -> None:
    ...
    assert mission["order_index"] == 39
    assert mission["flagship_version"] == "V10"
    assert mission["phase"]["id"] == "P7"
    assert mission["title"] == 'Add Memory, Routing, and Fallbacks'
    assert mission["prerequisites"] == ['M38']
...
def test_package_validates_against_wp136_and_g4_journey() -> None:
    result = validate_package(M39_PACKAGE)
...
def test_mdl_validator_accepts_sealed_package() -> None:
    ...
    payload = validate_mission(M39_PACKAGE / "missions" / "M39.json")
    assert payload["id"] == MISSION_ID
    package = mdl_validate_package(M39_PACKAGE)
    assert package.id == PACKAGE_ID
    loaded = g3_load(M39_PACKAGE)
...
def test_experiment_is_generic_runner_with_assistance_ladder() -> None:
    ...
    assert runner["module"] == "g5.reference.M39.experiment"
    assert runner["entrypoint"] == 'run_robust_agent'
...
def test_gate_contract_evidence_and_targeted_repair() -> None:
    ...
    assert comps == {'comp.robust.memory', 'comp.robust.routing', 'comp.robust.fallback', 'comp.robust.unassisted_transfer'}
    for node in ['kn.m39.memory', 'kn.m39.routing', 'kn.m39.fallback', 'kn.m39.no_ai_transfer']:
...
def test_package_has_no_prefilled_learner_evidence() -> None:
    blob = (M39_PACKAGE / "missions" / "M39.json").read_text(encoding="utf-8").lower()
    for forbidden in (
        "learner_evidence",
        "learner_response",
        "fabricated learner",
...
def test_platform_has_no_mission_special_case_routes_or_conditionals() -> None:
    ...
    eq_mid = re.compile(r"""mission_id\s*==\s*["']M39["']""")
    custom_route = re.compile(r"/m39[-_/]|memory-routing|robust-agent", re.IGNORECASE)
    hits: list[str] = []
...
def test_frontend_registry_stays_generic() -> None:
    ...
    assert "M39" not in registry
    assert 'robust_agent_memory' not in registry.lower()
...
def test_allowed_diff_paths_are_fixtures_and_reference_tests_only() -> None:
    ...
    assert any(path.startswith("platform/fixtures/M39/") for path in paths)
    assert any(path.startswith("tests/platform/reference/M39/") for path in paths)


def test_fixture_lives_only_in_allowed_g6_paths() -> None:
    relative = M39_PACKAGE.relative_to(REPO_ROOT)
    assert relative == Path("platform/fixtures/M39")
    tests = REPO_ROOT / "tests" / "platform" / "reference" / "M39"
    assert tests.is_dir()
