"""G5 M38 on the frozen generic runtime: predict → WP-137 execute → gate."""
...
PREDICT_BODY = {'hypothesis': 'SKU-7 lookup-then-post completes once; resume restores last_tool_result without replaying lookup; loop bound is 8', 'expected_values': {'terminal': 'complete', 'effect_count': 1, 'resume_replays_lookup': False}}

EXECUTE_CODE = """
mode = str(parameters.get("mode") or "happy")
max_steps = int(parameters.get("max_steps") or 8)
approve = bool(parameters.get("approve", True))
nodes = ["start", "decide", "validate", "execute", "assimilate"]
if mode == "invalid":
    terminal = "start"
    effect_count = 0
    raised = "InvalidTransition"
elif mode == "loop":
    nodes = ["start"] + ["decide"] * max_steps
    terminal = "loop_exhausted"
    effect_count = 0
    raised = None
elif mode == "deny" or not approve:
    nodes = ["start", "decide", "approve"]
    terminal = "denied"
    effect_count = 0
    raised = None
else:
    nodes = ["start", "decide", "validate", "execute", "assimilate", "complete"]
    terminal = "complete"
    effect_count = 1
    raised = None
payload = {
    ...
    "loop_bound": max_steps,
    "resume_replays_lookup": False,
    "last_tool_result": {"sku": "SKU-7", "price": 42.0} if effect_count else None,
    "raised": raised,
}
print(payload)
[
    {
        "type": "trace",
        "title": "workflow",
        "payload": {"steps": nodes, "terminal": terminal},
    },
    {"type": "metric", "title": "run", "payload": payload},
]
"""

EXECUTE_BODY = {"code": EXECUTE_CODE, "parameters": {'mode': 'happy', 'max_steps': 8, 'approve': True}}

SUBMIT_PLAN = [
    (STAGE_ORIENT, 'framed explicit serializable state before transitions; no hidden chat history', [{"artifact_type": "markdown"}]),
    (STAGE_CODE, 'schema, apply_transition, checkpoint/resume, approval, loop counter', [{"artifact_type": "trace"}]),
    (STAGE_EXPERIMENT, 'observed complete terminal after a sealed prediction; lookup not replayed', [{"artifact_type": 'trace'}]),
    (STAGE_FAILURE, 'missing loop bound exceeded steps; repair_run yielded loop_exhausted; keyed post', [{"artifact_type": "table"}]),
    (STAGE_TRANSFER, 'drew the machine without inventing a memory store; approval gates execute', [{"artifact_type": "artifact"}]),
    (STAGE_ADR, 'explicit state, checkpoints, loop_exhausted, approval node; no routing yet', [{"artifact_type": "markdown"}]),
    (STAGE_FLAGSHIP, 'M39 receives schema and termination semantics, not a LangGraph SDK graph', [{"artifact_type": "markdown"}]),
...
@pytest.fixture
def m38_env(monkeypatch):
    fastapi = pytest.importorskip("fastapi")
    del fastapi
    home = Path(tempfile.mkdtemp(prefix="los-g6-m38-home-", dir="/tmp"))
    sock = Path(f"/tmp/los-g6-m38-{uuid.uuid4().hex}.sock")
...
def test_sample_result_matches_frozen_wp137() -> None:
    ...
    sample = {
        "execution_id": "exec-m38-sample",
        "status": "SUCCESS",
        "exit_code": 0,
        "duration_ms": 12,
        "blocks": [{'type': 'trace', 'title': 'workflow', 'payload': {'steps': ['start', 'decide', 'execute', 'complete'], 'terminal': 'complete'}}, {'type': 'metric', 'title': 'run', 'payload': {'terminal': 'complete', 'effect_count': 1, 'resume_replays_lookup': False}}],
    }
...
def test_m38_runtime_roundtrip(m38_env):
    from fastapi.testclient import TestClient

    from app.main import app

    before = _home_mtime()
    home: Path = m38_env["home"]
    sock: Path = m38_env["sock"]
    env: dict[str, str] = m38_env["env"]
    ...
    try:
        ...
        with TestClient(app, client=("127.0.0.1", 50000)) as client:
            ...
            loaded = client.post(
                ...
                json={"package_dir": str(M38_PACKAGE)},
                headers=headers,
            )
            assert loaded.status_code == 200, loaded.text
            loaded_body = loaded.json()
            assert loaded_body.get("id") == "g5.reference.M38"
            ...
            assert "M38" in mission_ids
            assert "g5.reference.M38" not in mission_ids

            mission = client.get("/api/v1/missions/M38", headers=headers)
            assert mission.status_code == 200, mission.text
            spec = mission.json()
            assert spec.get("id") == "M38"
            ...
            learner = client.post(
                "/api/v1/learners",
                json={"username": "m38-learner", "display_name": "M38 Learner"},
                headers=headers,
            )
            ...
            session = client.post(
                "/api/v1/sessions",
                json={"mission_id": "M38", "learner_id": learner_id},
                headers=headers,
            )
            assert session.status_code == 200, session.text
            session_body = session.json()
            assert session_body["mission_id"] == "M38"
        ...
        with TestClient(app, client=("127.0.0.1", 50000)) as resumed:
            ...
            assert 'kn.m38.loop_approval' in nodes
            ...
            for stage_id, explanation, artifacts in SUBMIT_PLAN:
                ...
                assert metric['terminal'] == 'complete'
                assert metric['effect_count'] == 1
                assert metric['resume_replays_lookup'] == False
            ...
            assert increments == {'comp.agent.state_schema', 'comp.agent.checkpoint_resume', 'comp.agent.loop_approval', 'comp.agent.unassisted_transfer'}
