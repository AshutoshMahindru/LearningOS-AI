"""G6 M40 reference package: WP-136 spec, WP-137 rubric, generic runtime only."""
...
def _mission() -> dict:
    return json.loads((M40_PACKAGE / "missions" / "M40.json").read_text(encoding="utf-8"))


def test_package_identity_is_g6_reference() -> None:
    package = load_package(M40_PACKAGE)
...
def test_wp136_id_is_mission_not_package_id() -> None:
    ...
    assert mission["order_index"] == 40
    assert mission["flagship_version"] == 'V11'
    assert mission["phase"]["id"] == 'P7'
    assert mission["prerequisites"] == ['M34', 'M39']
...
def test_package_validates_against_wp136_and_g4_journey() -> None:
    result = validate_package(M40_PACKAGE)
...
def test_mdl_validator_accepts_sealed_package() -> None:
    ...
    payload = validate_mission(M40_PACKAGE / "missions" / "M40.json")
    assert payload["id"] == MISSION_ID
    package = mdl_validate_package(M40_PACKAGE)
    assert package.id == PACKAGE_ID
    loaded = g3_load(M40_PACKAGE)
...
def test_spec_and_rubric_use_wp137_block_types() -> None:
    ...
    for token in ('m40.eval.v1', 'citation', 'slice', 'grader', 'hidden-critical', 'contaminated', 'v11'):
...
def test_experiment_is_generic_runner_with_assistance_ladder() -> None:
    ...
    assert runner["module"] == 'g6.reference.M40.experiment'
    assert runner["entrypoint"] == 'run_eval_suite'
...
def test_gate_contract_evidence_and_targeted_repair() -> None:
    ...
    assert comps == {'comp.eval.slice_gates', 'comp.eval.unassisted_transfer', 'comp.eval.deterministic_graders', 'comp.eval.suite_versioning'}
    for node in ('kn.m40.eval_versioning', 'kn.m40.deterministic_graders', 'kn.m40.hidden_critical', 'kn.m40.no_ai_transfer'):
        assert node in mission["knowledge_nodes"]


def test_package_has_no_prefilled_learner_evidence() -> None:
    blob = (M40_PACKAGE / "missions" / "M40.json").read_text(encoding="utf-8").lower()
    for forbidden in (
        "learner_evidence",
        "learner_response",
        "fabricated learner",
...
def test_package_has_no_custom_frontend_or_eager_pandas() -> None:
    ...
    assert f"/missions/m40/" not in mission
...
def test_no_eager_pandas_import_in_fixture_or_tests() -> None:
    roots = [M40_PACKAGE, Path(__file__).resolve().parent]
    for root in roots:
...
def test_platform_has_no_mission_special_case_routes_or_conditionals() -> None:
    ...
    eq_mid = re.compile(r"""mission_id\s*==\s*["']M40["']""")
    custom_route = re.compile(r"/m40[-_/]|ai-system-evals|eval-harness", re.IGNORECASE)
    hits: list[str] = []
...
def test_allowed_diff_paths_are_fixtures_and_reference_tests_only() -> None:
    ...
    assert any(path.startswith("platform/fixtures/M40/") for path in paths)
    assert any(path.startswith("tests/platform/reference/M40/") for path in paths)


def test_fixture_lives_only_in_allowed_g6_paths() -> None:
    relative = M40_PACKAGE.relative_to(REPO_ROOT)
    assert relative == Path("platform/fixtures/M40")
    tests = REPO_ROOT / "tests" / "platform" / "reference" / "M40"
    assert tests.is_dir()
