"""G5 M22 reference package: WP-136 spec, WP-137 rubric, generic runtime only."""
...
def _mission() -> dict:
    return json.loads((M22_PACKAGE / "missions" / "M22.json").read_text(encoding="utf-8"))


def test_package_identity_is_g5_reference_m22() -> None:
    package = load_package(M22_PACKAGE)
...
def test_package_validates_against_wp136_and_g4_journey() -> None:
    result = validate_package(M22_PACKAGE)
...
def test_mdl_validator_accepts_sealed_package() -> None:
    ...
    payload = validate_mission(M22_PACKAGE / "missions" / "M22.json")
    assert payload["id"] == MISSION_ID
    package = mdl_validate_package(M22_PACKAGE)
    assert package.id == PACKAGE_ID
    loaded = g3_load(M22_PACKAGE)
...
def test_experiment_is_catalogue_oriented_on_generic_runner() -> None:
    ...
    assert runner["module"] == "g5.reference.M22.experiment"
    assert runner["entrypoint"] == "run_neuron_layer"
...
def test_gate_contract_evidence_and_targeted_repair() -> None:
    ...
    assert set(comps) == {'comp.nn.activation_maps', 'comp.nn.dense_layer_shapes', 'comp.nn.layer_boundary_repair', 'comp.nn.neuron_arithmetic'}
...
def test_package_has_no_prefilled_learner_evidence() -> None:
    blob = (M22_PACKAGE / "missions" / "M22.json").read_text(encoding="utf-8").lower()
    for forbidden in (
        "learner_evidence",
        "learner_response",
        "fabricated learner",
...
def test_source_mission_content_is_represented() -> None:
    ...
    for token in ('predict', 'relu', 'sigmoid', 'tanh', 'affine', 'pre-activation', 'no custom', 'v05', 'bias'):
        assert token in mission, token


def test_package_has_no_custom_frontend_or_runtime_payload() -> None:
    for path in M22_PACKAGE.rglob("*"):
        if not path.is_file():
            continue
...
def test_platform_has_no_m22_special_case_routes_or_conditionals() -> None:
    platform = REPO_ROOT / "platform"
    skip_dirs = {"node_modules", "dist", "__pycache__", ".git"}
    eq_mid = re.compile(r"""mission_id\s*==\s*["']M22["']""")
    custom_route = re.compile(r"/m22[-_/]|/neuron-layer", re.IGNORECASE)
    hits: list[str] = []
    for path in platform.rglob("*"):
        ...
        if any(part in skip_dirs for part in path.parts):
            continue
        if "fixtures" in path.parts and "M22" in path.parts:
            continue
...
def test_frontend_registry_stays_generic() -> None:
    ...
    assert "M22" not in registry
    assert "neuron_layer" not in registry.lower()
...
def _lane_commits() -> list[str]:
    ...
    listed = subprocess.run(
        [
            ...
            f"{FROZEN_BASE}..HEAD",
            "--",
            f"platform/fixtures/M22",
            f"tests/platform/reference/M22",
        ],
        cwd=str(REPO_ROOT),
        check=False,
        capture_output=True,
    ...
    if not shas:
        pytest.skip(f"no lane commits for M22 versus frozen base in this clone")
    return shas
...
def test_allowed_diff_paths_are_fixtures_and_reference_tests_only() -> None:
    ...
    for sha in shas:
        ...
        for path in completed.stdout.splitlines():
            ...
            if path.startswith("platform/fixtures/M22/"):
                saw_fixture = True
            if path.startswith("tests/platform/reference/M22/"):
                saw_tests = True
