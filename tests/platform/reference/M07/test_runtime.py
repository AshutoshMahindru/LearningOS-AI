"""G6 M07 on the frozen generic runtime: predict seal, no-AI, evidence, gate, repair."""
...
PACKAGE = REPO_ROOT / "platform" / "fixtures" / "M07"
...
PREDICT_BODY = {"hypothesis": "independent get_dummies on unseen categories changes width; fitted pipeline keeps width", "expected_values": {"mismatch": True, "repair_width": 2}}

EXECUTE_CODE = """
train_cols = ['cat_a', 'cat_b']
infer_cols = ['cat_a', 'cat_b', 'cat_c']
payload = {
    'train_width': len(train_cols),
    'infer_width': len(infer_cols),
    'mismatch': len(train_cols) != len(infer_cols),
    'repair_width': len(train_cols),
    'reload_equal': True,
}
print(payload)
{'type': 'metric', 'title': 'encoding-boundary', 'payload': payload}
"""

EXECUTE_BODY = {"code": EXECUTE_CODE, "parameters": {}}
EXPECTED_BLOCK = "metric"
REPAIR_NODE = "kn.m07.fit_transform_boundary"
REPAIR_STAGE = "stage_02_code_reading"
...
def env_home(monkeypatch):
    ...
    del fastapi
    home = Path(tempfile.mkdtemp(prefix="los-g6-m07-home-", dir="/tmp"))
    sock = Path(f"/tmp/los-g6-m07-{uuid.uuid4().hex}.sock")
...
def _load_and_start(client, headers):
    ...
    learner = client.post(
        "/api/v1/learners",
        json={"username": "m07-learner", "display_name": "M07 Learner"},
        headers=headers,
    )
