...
MISSION_PATH = PACKAGE / "missions" / "M07.json"
...
FLAGSHIP = "V01"
PHASE_ID = "P1"
TITLE = "Build a Reusable Data-to-Model Pipeline"
FROZEN_STAGE_TYPES = {
    "orientation",
    "trace_map",
    "interrogate",
...
SOURCE_TOKENS = ["pipeline", "fit", "transform", "get_dummies", "handle_unknown", "serialize", "customer", "v01"]
...
def test_package_layout_and_identity() -> None:
    ...
    assert manifest["missions"][0]["path"] == "missions/M07.json"
...
def test_integrity_files_match_payload() -> None:
    ...
    assert "missions/M07.json" in checksums
...
def test_prediction_cycle_on_every_experiment_stage() -> None:
    ...
    for stage in experiments:
        ...
        assert runner.get("module", "").startswith("g6.reference.M07.")
...
def test_package_has_no_custom_frontend_or_runtime_payload() -> None:
    ...
    mission = json.dumps(_mission()).lower()
    assert "/missions/m07/" not in mission
...
def test_platform_has_no_mission_special_case_routes() -> None:
    ...
    eq_mission = re.compile(r"""==\s*["']M07["']""")
    route_mission = re.compile(r"/missions/M07")
    hits: list[str] = []
    for root in roots:
        if not root.exists():
            continue
...
def test_fixture_lives_only_in_allowed_g6_paths() -> None:
    ...
    assert relative == Path("platform/fixtures/M07")
    tests = REPO_ROOT / "tests" / "platform" / "reference" / "M07"
    assert tests.is_dir()
