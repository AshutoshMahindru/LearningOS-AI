"""G6 M42 reference package: WP-136 spec, WP-137 rubric, generic runtime only."""
...
LANE_ALLOWED = tuple(
    f"platform/fixtures/M{i:02d}/" for i in range(40, 43)
) + tuple(
    f"tests/platform/reference/M{i:02d}/" for i in range(40, 43)
)


def _mission() -> dict:
    return json.loads((M42_PACKAGE / "missions" / "M42.json").read_text(encoding="utf-8"))


def test_package_identity_is_g6_reference() -> None:
    package = load_package(M42_PACKAGE)
...
def test_wp136_id_is_mission_not_package_id() -> None:
    ...
    assert mission["order_index"] == 42
    assert mission["flagship_version"] == 'V12'
    assert mission["phase"]["id"] == 'P8'
    assert mission["prerequisites"] == ['M41']
...
def test_package_validates_against_wp136_and_g4_journey() -> None:
    result = validate_package(M42_PACKAGE)
...
def test_mdl_validator_accepts_sealed_package() -> None:
    ...
    payload = validate_mission(M42_PACKAGE / "missions" / "M42.json")
    assert payload["id"] == MISSION_ID
    package = mdl_validate_package(M42_PACKAGE)
    assert package.id == PACKAGE_ID
    loaded = g3_load(M42_PACKAGE)
...
def test_spec_and_rubric_use_wp137_block_types() -> None:
    ...
    for token in ('capstone', 'orchestrat', 'degrad', 'v12', 'wp-136', 'one-off', 'generic runtime'):
...
def test_experiment_is_generic_runner_with_assistance_ladder() -> None:
    ...
    assert runner["module"] == 'g6.reference.M42.experiment'
    assert runner["entrypoint"] == 'run_capstone_orchestrator'
...
def test_gate_contract_evidence_and_targeted_repair() -> None:
    ...
    assert comps == {'comp.cap.orchestration', 'comp.cap.unassisted_defense', 'comp.cap.degradation_defense', 'comp.cap.evaluation_reuse'}
    for node in ('kn.m42.end_to_end_orchestrator', 'kn.m42.m40_eval_reuse', 'kn.m42.degradation_and_sla', 'kn.m42.no_ai_defense'):
        assert node in mission["knowledge_nodes"]


def test_package_has_no_prefilled_learner_evidence() -> None:
    blob = (M42_PACKAGE / "missions" / "M42.json").read_text(encoding="utf-8").lower()
    for forbidden in (
        "learner_evidence",
        "learner_response",
        "fabricated learner",
...
def test_platform_has_no_mission_special_case_routes_or_conditionals() -> None:
    ...
    eq_mid = re.compile(r"""mission_id\s*==\s*["']M42["']""")
    custom_route = re.compile(r"/m42[-_/]|capstone-app|autonomous-enterprise", re.IGNORECASE)
    hits: list[str] = []
...
def _changed_paths() -> list[str]:
    ...
    if any(path.startswith("platform/fixtures/M42/") for path in committed):
        return committed
    extra = [
        path
...
def test_allowed_diff_paths_are_fixtures_and_reference_tests_only() -> None:
    ...
    assert any(path.startswith("platform/fixtures/M42/") for path in paths)
    assert any(path.startswith("tests/platform/reference/M42/") for path in paths)


def test_fixture_lives_only_in_allowed_g6_paths() -> None:
    relative = M42_PACKAGE.relative_to(REPO_ROOT)
    assert relative == Path("platform/fixtures/M42")
    tests = REPO_ROOT / "tests" / "platform" / "reference" / "M42"
    assert tests.is_dir()


def test_capstone_is_wp136_package_not_one_off_app() -> None:
    package = load_package(M42_PACKAGE)
    mission = _mission()
    assert package.id == PACKAGE_ID
    assert mission["id"] == MISSION_ID
    blob = json.dumps(mission).lower()
    assert "wp-136" in blob
    assert "not a one-off app" in blob
    assert "missions.m42.lab" not in blob
    assert "custom frontend" in (package.manifest.get("description") or "").lower()
    for item in mission["gate_contract"]["required_evidence"]:
        assert item["artifact_type"] in WP137_BLOCK_TYPES
        assert item["artifact_type"] not in {"code", "adr"}
    types = [stage["type"] for stage in mission["stages"]]
