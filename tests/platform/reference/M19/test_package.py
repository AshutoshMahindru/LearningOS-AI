"""G6 M19 reference package: WP-136 spec, WP-137 rubric, generic runtime only."""
...
M19_PACKAGE = REPO_ROOT / "platform" / "fixtures" / "M19"
FROZEN_BASE = "f7926e661a955f2d78bd8584877815825c5ef047"
PACKAGE_ID = "g6.reference.M19"
MISSION_ID = "M19"
WP137_BLOCK_TYPES = {
    "table",
    "chart",
...
def _mission() -> dict:
    return json.loads((M19_PACKAGE / "missions" / "M19.json").read_text(encoding="utf-8"))


def test_package_identity_is_g6_reference() -> None:
    package = load_package(M19_PACKAGE)
...
def test_wp136_id_is_mission_not_package_id() -> None:
    ...
    assert mission["order_index"] == 19
    assert mission["flagship_version"] == "V04"
    assert mission["prerequisites"] == ["M16", "M17"]
...
def test_package_validates_against_wp136_and_g4_journey() -> None:
    result = validate_package(M19_PACKAGE)
...
def test_mdl_validator_accepts_sealed_package() -> None:
    ...
    payload = validate_mission(M19_PACKAGE / "missions" / "M19.json")
    assert payload["id"] == MISSION_ID
    package = mdl_validate_package(M19_PACKAGE)
    assert package.id == PACKAGE_ID
    loaded = g3_load(M19_PACKAGE)
...
def test_spec_and_rubric_use_wp137_block_types() -> None:
    ...
    assert "gradient" in blob
    for token in ('descent', 'finite-difference', 'wrong-sign'):
...
def test_experiment_is_generic_runner_with_assistance_ladder() -> None:
    ...
    assert runner["module"] == "g6.reference.M19.experiment"
    assert runner["entrypoint"] == "run_one_step_update"
...
def test_gate_contract_evidence_and_targeted_repair() -> None:
    ...
    assert comps == {'comp.grad.loss_curve', 'comp.grad.update', 'comp.grad.sign_diagnosis', 'comp.grad.unassisted_transfer'}
    for node in ('kn.m19.parameter_to_loss', 'kn.m19.descent_update', 'kn.m19.wrong_sign', 'kn.m19.no_ai_transfer'):
        assert node in mission["knowledge_nodes"]


def test_package_has_no_prefilled_learner_evidence() -> None:
    blob = (M19_PACKAGE / "missions" / "M19.json").read_text(encoding="utf-8").lower()
    for forbidden in (
        "learner_evidence",
        "learner_response",
        "fabricated learner",
...
def test_package_has_no_custom_frontend_or_eager_pandas() -> None:
    ...
    assert f"/missions/m19/" not in mission
...
def test_platform_has_no_mission_special_case_routes_or_conditionals() -> None:
    ...
    eq_mid = re.compile(r"""mission_id\s*==\s*["']M19["']""")
    custom_route = re.compile(r"/m19[-_/]|gradients|wrong-sign", re.IGNORECASE)
    hits: list[str] = []
...
def _changed_paths() -> list[str]:
    ...
    if any(path.startswith("platform/fixtures/M19/") for path in committed):
        return committed
    extra = [
        path
...
def test_allowed_diff_paths_are_fixtures_and_reference_tests_only() -> None:
    ...
    assert any(path.startswith("platform/fixtures/M19/") for path in paths)
    assert any(path.startswith("tests/platform/reference/M19/") for path in paths)


def test_fixture_lives_only_in_allowed_g6_paths() -> None:
    relative = M19_PACKAGE.relative_to(REPO_ROOT)
    assert relative == Path("platform/fixtures/M19")
    tests = REPO_ROOT / "tests" / "platform" / "reference" / "M19"
    assert tests.is_dir()
