"""G5 M36 on the frozen generic runtime: predict → WP-137 execute → gate."""
...
M36_PACKAGE = REPO_ROOT / "platform" / "fixtures" / "M36"
...
PREDICT_BODY = {'hypothesis': 'RRF over dense and sparse ranks recovers gold; exact remains the oracle; pre-filter keeps eligible relevant that late small top-k drops', 'expected_values': {'fusion': 'rrf', 'exact_is_oracle': True, 'prefilter': True}}

EXECUTE_CODE = """
rrf_k = int(parameters.get("rrf_k") or 60)
mix_raw = bool(parameters.get("mix_raw") or 0)
dense = ["doc-account-access::c0", "doc-device-printer::c0", "doc-account-access::c1"]
sparse = ["doc-account-access::c1", "doc-account-access::c0", "doc-device-printer::c0"]
...
if mix_raw:
    fused = ["doc-device-printer::c0", "doc-account-access::c0", "doc-account-access::c1"]
    fusion = "raw_sum"
else:
...
payload = {
    "fusion": fusion,
    "rrf_k": rrf_k,
    "exact_is_oracle": True,
    "prefilter": True,
    "fused_top": fused[0],
    "gold_recovered": fused[0].startswith("doc-account-access"),
}
print(payload)
[
    {
        ...
        "title": "channel-ranks",
        "payload": {
            "columns": ["channel", "rank1", "rank2"],
            "rows": [["dense", dense[0], dense[1]], ["sparse", sparse[0], sparse[1]], ["fused", fused[0], fused[1]]],
        },
    },
    {"type": "metric", "title": "hybrid", "payload": payload},
]
"""

EXECUTE_BODY = {"code": EXECUTE_CODE, "parameters": {'rrf_k': 60, 'mix_raw': 0}}

SUBMIT_PLAN = [
    (STAGE_ORIENT, 'framed exact oracle versus teaching graph; gate needs RRF, filters, lifecycle', [{"artifact_type": "markdown"}]),
    (STAGE_CODE, 'store schema, ef, pre-filter, BM25, RRF, dirty rebuild; fusion field equals rrf', [{"artifact_type": "trace"}]),
    (STAGE_EXPERIMENT, 'observed declared RRF after a sealed prediction; exact stayed the oracle', [{"artifact_type": 'table'}]),
    (STAGE_FAILURE, 'raw cosine+BM25 mix promoted a trap; repaired with RRF from broken channel objects', [{"artifact_type": "table"}]),
    (STAGE_TRANSFER, 'hand RRF without inventing a cluster; late-filter miss named; exact when correctness binds', [{"artifact_type": "artifact"}]),
    (STAGE_ADR, 'exact teaching oracle; optional ANN; declared RRF; fail-closed dirty store', [{"artifact_type": "markdown"}]),
    (STAGE_FLAGSHIP, 'V09 closes with architecture plus measurable retrieval for M40, not a cluster', [{"artifact_type": "markdown"}]),
...
@pytest.fixture
def m36_env(monkeypatch):
    fastapi = pytest.importorskip("fastapi")
    del fastapi
    home = Path(tempfile.mkdtemp(prefix="los-g6-m36-home-", dir="/tmp"))
    sock = Path(f"/tmp/los-g6-m36-{uuid.uuid4().hex}.sock")
...
def test_sample_result_matches_frozen_wp137() -> None:
    ...
    sample = {
        "execution_id": "exec-m36-sample",
        "status": "SUCCESS",
        "exit_code": 0,
        "duration_ms": 12,
        "blocks": [{'type': 'table', 'title': 'channel-ranks', 'payload': {'columns': ['channel', 'rank1', 'rank2'], 'rows': [['dense', 'doc-account-access::c0', 'doc-device-printer::c0']]}}, {'type': 'metric', 'title': 'hybrid', 'payload': {'fusion': 'rrf', 'exact_is_oracle': True, 'prefilter': True}}],
    }
...
def test_m36_runtime_roundtrip(m36_env):
    from fastapi.testclient import TestClient

    from app.main import app

    before = _home_mtime()
    home: Path = m36_env["home"]
    sock: Path = m36_env["sock"]
    env: dict[str, str] = m36_env["env"]
    ...
    try:
        ...
        with TestClient(app, client=("127.0.0.1", 50000)) as client:
            ...
            loaded = client.post(
                ...
                json={"package_dir": str(M36_PACKAGE)},
                headers=headers,
            )
            assert loaded.status_code == 200, loaded.text
            loaded_body = loaded.json()
            assert loaded_body.get("id") == "g5.reference.M36"
            ...
            assert "M36" in mission_ids
            assert "g5.reference.M36" not in mission_ids

            mission = client.get("/api/v1/missions/M36", headers=headers)
            assert mission.status_code == 200, mission.text
            spec = mission.json()
            assert spec.get("id") == "M36"
            ...
            learner = client.post(
                "/api/v1/learners",
                json={"username": "m36-learner", "display_name": "M36 Learner"},
                headers=headers,
            )
            ...
            session = client.post(
                "/api/v1/sessions",
                json={"mission_id": "M36", "learner_id": learner_id},
                headers=headers,
            )
            assert session.status_code == 200, session.text
            session_body = session.json()
            assert session_body["mission_id"] == "M36"
        ...
        with TestClient(app, client=("127.0.0.1", 50000)) as resumed:
            ...
            assert 'kn.m36.hybrid_fusion' in nodes
            ...
            for stage_id, explanation, artifacts in SUBMIT_PLAN:
                ...
                assert metric['fusion'] == 'rrf'
                assert metric['exact_is_oracle'] == True
                assert metric['prefilter'] == True
            ...
            assert increments == {'comp.store.exact_oracle', 'comp.store.hybrid_fusion', 'comp.store.filter_lifecycle', 'comp.store.unassisted_transfer'}
