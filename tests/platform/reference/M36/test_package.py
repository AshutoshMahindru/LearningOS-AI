"""G5 M36 reference package: WP-136 spec, WP-137 rubric, generic runtime only."""
...
SOURCE_TOKENS = ['predict', 'hybrid', 'rrf', 'approximate', 'filter', 'v09', 'no custom', 'exact', 'lifecycle']


def _mission() -> dict:
    return json.loads((M36_PACKAGE / "missions" / "M36.json").read_text(encoding="utf-8"))


def test_package_identity_is_g5_reference() -> None:
    package = load_package(M36_PACKAGE)
...
def test_wp136_id_is_mission_not_package_id() -> None:
    ...
    assert mission["order_index"] == 36
    assert mission["flagship_version"] == "V09"
    assert mission["phase"]["id"] == "P6"
    assert mission["title"] == 'Understand Vector Databases and Hybrid Retrieval'
    assert mission["prerequisites"] == ['M35']
...
def test_package_validates_against_wp136_and_g4_journey() -> None:
    result = validate_package(M36_PACKAGE)
...
def test_mdl_validator_accepts_sealed_package() -> None:
    ...
    payload = validate_mission(M36_PACKAGE / "missions" / "M36.json")
    assert payload["id"] == MISSION_ID
    package = mdl_validate_package(M36_PACKAGE)
    assert package.id == PACKAGE_ID
    loaded = g3_load(M36_PACKAGE)
...
def test_experiment_is_generic_runner_with_assistance_ladder() -> None:
    ...
    assert runner["module"] == "g5.reference.M36.experiment"
    assert runner["entrypoint"] == 'run_hybrid_fusion'
...
def test_gate_contract_evidence_and_targeted_repair() -> None:
    ...
    assert comps == {'comp.store.exact_oracle', 'comp.store.hybrid_fusion', 'comp.store.filter_lifecycle', 'comp.store.unassisted_transfer'}
    for node in ['kn.m36.exact_oracle', 'kn.m36.hybrid_fusion', 'kn.m36.filter_lifecycle', 'kn.m36.no_ai_transfer']:
...
def test_package_has_no_prefilled_learner_evidence() -> None:
    blob = (M36_PACKAGE / "missions" / "M36.json").read_text(encoding="utf-8").lower()
    for forbidden in (
        "learner_evidence",
        "learner_response",
        "fabricated learner",
...
def test_platform_has_no_mission_special_case_routes_or_conditionals() -> None:
    ...
    eq_mid = re.compile(r"""mission_id\s*==\s*["']M36["']""")
    custom_route = re.compile(r"/m36[-_/]|hybrid-retrieval|teaching-graph", re.IGNORECASE)
    hits: list[str] = []
...
def test_frontend_registry_stays_generic() -> None:
    ...
    assert "M36" not in registry
    assert 'hybrid_vector_db' not in registry.lower()
...
def test_allowed_diff_paths_are_fixtures_and_reference_tests_only() -> None:
    ...
    assert any(path.startswith("platform/fixtures/M36/") for path in paths)
    assert any(path.startswith("tests/platform/reference/M36/") for path in paths)


def test_fixture_lives_only_in_allowed_g6_paths() -> None:
    relative = M36_PACKAGE.relative_to(REPO_ROOT)
    assert relative == Path("platform/fixtures/M36")
    tests = REPO_ROOT / "tests" / "platform" / "reference" / "M36"
    assert tests.is_dir()
